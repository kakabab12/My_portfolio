...
def init_camera(config, device_id=None):
    ...
    _apply_exposure(cap, config)
    _log_camera_negotiation(cap, config, device_id)
    return cap


def _apply_exposure(cap, config):
    """노출·게인 수동 설정 — **먼 사람을 광학으로 지우는** 수단 (2026-08-07 신설).

    사용자 제안(물류창고 PDA 바코드 스캐너)에서 온 기능이다. 스캐너가 어두운
    창고에서도 가까운 바코드만 정확히 읽는 이유는 알고리즘이 아니라 광학이다 —
    **자기 조명을 쏘고 노출을 짧게 잡으면**, 빛이 거리 제곱에 반비례해 감쇠하므로
    (1/d²) 가까운 피사체만 밝게 찍히고 먼 배경은 거의 검게 떨어진다.
    엔진이 오늘 하루 소프트웨어로 싸운 "뒷사람" 문제를 **이미지 단계에서**
    없애는 방법이다 — 안 찍힌 사람은 검출될 수 없다.

    ★지금(실내 조명·RGB 웹캠)은 효과가 제한적이다: 실내광이 균일하면 뒷사람도
    같이 밝다. 이 기능이 값을 하는 건 **조명이 카메라 쪽에 있을 때**다 —
    키오스크 전면 조명, 그리고 무엇보다 예정된 IR 카메라 전환
    (docs/TODO.md "본 개발은 IR카메라로 변경됨")이다. IR은 조명이 장치에
    딸려오므로 위 1/d² 감쇠가 그대로 성립한다. 그때 쓰라고 미리 배선해 둔다.

    키(camera.exposure)가 없으면 아무것도 안 건드린다 — 종전 동작(자동 노출).
    ※장치가 지원하지 않는 속성은 cap.set이 조용히 False를 돌려줄 뿐 예외가
    나지 않는다. 요청과 실제가 다를 수 있어 결과를 로그로 남긴다(아래 협상 로그와
    같은 취지) — 설정이 먹었는지 실기에서 확인할 수 있어야 한다.
    """
    exposure_cfg = config["camera"].get("exposure") or {}
    if not exposure_cfg.get("manual"):
        return
    # 0.25 = 수동(DSHOW 관습), 0.75 = 자동. 백엔드마다 값이 달라 config로 뺀다
    cap.set(cv2.CAP_PROP_AUTO_EXPOSURE, exposure_cfg.get("auto_exposure_off_value", 0.25))
    if exposure_cfg.get("exposure_value") is not None:
        cap.set(cv2.CAP_PROP_EXPOSURE, exposure_cfg["exposure_value"])
    if exposure_cfg.get("gain") is not None:
        cap.set(cv2.CAP_PROP_GAIN, exposure_cfg["gain"])
    logger.info(
        "노출 수동 설정: 요청 exposure=%s gain=%s -> 실제 exposure=%.3f gain=%.3f "
        "(장치가 무시하면 값이 다르다)",
        exposure_cfg.get("exposure_value"), exposure_cfg.get("gain"),
        cap.get(cv2.CAP_PROP_EXPOSURE), cap.get(cv2.CAP_PROP_GAIN))
