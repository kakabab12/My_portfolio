...
class CursorMappingTest(HeadTrackerTestBase):
    ...
    def test_cursor_moves_with_nose_offset(self):
        ...
        result = self.tracker.update(make_face(nose_px=(120.0, 100.0)))
    ...
    def test_calibration_median_resists_single_outlier_frame(self):
        ...
        for nose_y in nose_ys:
            result = self.tracker.update(make_face(nose_px=(100.0, nose_y)))
            if result.cursor_x_ratio is not None:
                break
            self.clock.tick(FRAME_DT_SEC)
...
class RecenterDwellTest(HeadTrackerTestBase):
    ...
    def test_moving_away_resets_progress(self):
        self._settle_calibration()
        self.tracker.update(make_face(nose_px=(100.0, 100.0)))
        self.clock.tick(0.2)
        moved = make_face(nose_px=(400.0, 100.0))   # 반경 이탈 — 앵커 재설정
