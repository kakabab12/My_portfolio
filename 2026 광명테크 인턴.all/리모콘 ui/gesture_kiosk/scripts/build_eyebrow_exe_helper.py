"""eyebrow.exe 빌드 로직 — make_eyebrow_exe.bat이 이 스크립트 하나만 호출한다
(2026-08-13 신설, scripts/build_head_exe_helper.py와 동일 패턴 — 그 파일
독스트링 참고).

PyInstaller는 시스템 파이썬에 깔지 않는다 — 빌드 전용 임시 venv에만 설치하고
끝나면 지운다. 결과물(eyebrow.exe)만 프로젝트 루트에 남는다. 다른 exe 빌드와
...
BUILD_VENV_DIR = os.path.join(ROOT_DIR, ".build_venv_tmp_eyebrow")
...
def _build_exe(build_python):
    ...
    print("[INFO] eyebrow.exe 빌드 중...")
    cmd = [
        build_python, "-m", "PyInstaller", "--onefile", "--console", "--name", "eyebrow",
        "--distpath", ROOT_DIR,
        "--workpath", os.path.join(BUILD_VENV_DIR, "work"),
        "--specpath", BUILD_VENV_DIR,
        os.path.join("scripts", "eyebrow_launcher.py"),
    ]
...
def main():
    ...
    print("[DONE] eyebrow.exe 생성 완료 - 프로젝트 루트에서 확인하세요")
    return 0
