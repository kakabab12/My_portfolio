...
D-pad는 화면 비율에 맞춘 **고정 위치·작은 크기**로 화면 우상단 구석에
그린다(2026-08-04 사용자 결정 — 화면을 거의 채우는 중앙 대형판은
dpad_ui_demo_fixed_center.py에, 손을 따라다니던 판은
dpad_ui_demo_palm_follow.py에 보존. "왼쪽에 조금 작게" -> "오른쪽 위 구석"
순서로 위치가 바뀌었다).
...
CURSOR_SENSITIVITY = 1.0   # 손끝이 화면 중앙에서 벗어난 픽셀 오프셋 -> 커서 오프셋
                           # 배율. 1.0=그대로 옮김. 존까지 못 닿으면 ↑, 너무 예민하면 ↓
...
def main():
    ...
    parser.add_argument("--cx-ratio", type=float, default=0.80, help="D-pad 중심 X (화면 폭 비율) — 기본 우측")
    ...
    logger.warning("D-pad UI demo started - fixed in the corner, cursor follows your "
                   "hand. Point toward a direction for %.2fs to fire it. Quit: q or ESC",
                   args.dwell_sec)
    ...
    try:
        while True:
            ...
            # 커서 = 손끝의 절대 좌표가 아니라 "화면 중앙(평상시 손 위치로 가정)
            # 대비 오프셋"을 D-pad 중심에 그대로 옮겨 찍는다 — D-pad가 구석에
            # 작게 있어도 몸 앞 편한 위치에서 손을 움직이면 커서가 D-pad 근처에서
            # 그 방향으로 움직인다 (2026-08-04 사용자 요청)
            cursor_px = None
            zone = None
            if fingertip_px is not None:
                frame_center_px = (w_px / 2.0, h_px / 2.0)
                dx_px = (fingertip_px[0] - frame_center_px[0]) * CURSOR_SENSITIVITY
                dy_px = (fingertip_px[1] - frame_center_px[1]) * CURSOR_SENSITIVITY
