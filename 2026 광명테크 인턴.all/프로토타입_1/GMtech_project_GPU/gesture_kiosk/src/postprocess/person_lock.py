...
class PersonLock:
    def __init__(self, config, frame_width_px, clock=time.monotonic, sharpness_fn=None):
        ...
        # 2026-07-15: 포즈 워커 스레드(realtime_loop._start_pose_worker, 쓰기)와 메인
        # 추론 루프(읽기)가 동시에 접근한다 — RLock인 이유는 attach_detections()가 잠금
        # 상태에서 user_wrists()를 다시 부르는 재귀 호출이 있어서(일반 Lock이면 교착)
