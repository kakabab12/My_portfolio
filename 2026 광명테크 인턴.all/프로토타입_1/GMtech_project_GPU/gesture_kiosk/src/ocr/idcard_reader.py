...
class IdCardReader:
    ...
    def _ensure_reader(self):
        ...
        try:
            ...
            if self._use_gpu:
                # Reader() 생성 성공 != GPU에서 실제로 돎. torch가 이 GPU의 컴퓨트
                # capability용 커널을 안 갖고 있으면(예: MX250·Pascal, 2026-07-15 실측 —
                # "cuDNN version ... is not compatible with devices with SM < 7.5") 생성은
                # 조용히 성공하고 첫 readtext()에서만 RuntimeError가 난다. 더미로 미리
                # 확인해 여기서 못 잡으면 실사용 중 OCR이 매번 조용히 실패하게 된다
                self._reader.readtext(np.zeros((64, 64, 3), dtype=np.uint8))
        except Exception:  # GPU 미지원(오래된 GPU·개발 PC 등)이면 CPU로 폴백
            logger.warning("EasyOCR GPU 초기화/추론 실패 — CPU로 폴백합니다", exc_info=True)
