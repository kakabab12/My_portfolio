...
try:
    ...
    print(f"   진료과: {sorted(pcs_lookup['department_name'].unique())}")
...
def fmt_val(feat, val):
    ...
    if feat == 'department':
        ...
        dept_kr = {'GS':'일반외과','CTS':'흉부외과','NS':'신경외과','OS':'정형외과',
                   'OG':'산부인과','UR':'비뇨기과','PS':'성형외과','OL':'안과','OT':'기타'}
        return dept_kr.get(orig, orig)
    if feat == 'antype':
        return inverse_encoders.get('antype', {}).get(int(val), str(int(val)))
    if feat == 'icd10_pcs':
...
def get_surgeries():
    ...
    for _, r in filtered.iterrows():
        ...
        desc = str(r['desc']) if 'desc' in r.index and str(r.get('desc','')) not in ('', 'nan') else ''
