...
def predict():
    ...
    # ASA: '2단계 — 경미한 질환' 같은 텍스트에서 숫자만 추출
    asa_raw = str(d.get('asa', '') or '')
    asa_match = re.search(r'\d+', asa_raw)
    row['asa'] = float(asa_match.group()) if asa_match else float(medians.get('asa', 2))
