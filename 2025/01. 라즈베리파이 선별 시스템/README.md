...
결과를 아두이노로 보내 **분류기를 제어**하는 시스템이다.
**그래픽카드가 없는 라즈베리파이에서 YOLOv8-seg를 CPU로** 돌리는 것이 핵심 과제였다.
...
**같은 기능의 세 세대**가 남아 있어 개선 과정을 볼 수 있다.
...
| `FastAPI_GPU버전-수정본/camera.py` | **하이브리드 카메라 래퍼.** RealSense 연결을 먼저 시도하고 **실패하면 자동으로 USB 웹캠으로 폴백**하는 고가용성 구조다. 백그라운드 스레드로 읽어 서버 부하를 줄인다 |
...
★ 이때 겪은 **"자원이 부족한 보드에서 AI를 어떻게 돌릴 것인가"** 문제의식이 이후
[2026년 캡스톤의 TensorRT 최적화](../../2026/03.%20캡스톤%20-%20스마트팩토리%20비전검사/)와
[광명테크 인턴의 OpenVINO 전환](../../2026/01.%20광명테크%20인턴%20-%20배리어프리%20키오스크/)으로 이어졌다.
