...
# 이 값(0.88)보다 작으면 무조건 불량으로 처리됨
RECTANGULARITY_THRESHOLD = 0.88

# [⚡ 속도 최적화]
VOTE_BUFFER_SIZE = 7       # 인식 안정화 버퍼 (숫자가 작을수록 반응 빠름)
...
def read_from_arduino():
    ...
    while True:
        try:
            if ser is None:
                # 포트 자동 연결 시도 (안 되면 직접 지정: '/dev/ttyACM0')
        ...
        for r in results:
            if r.masks is not None:
                ...
                for mask, box in zip(masks, boxes):
                    ...
                    # (1) YOLO가 원래 불량이라고 했는지 확인
                    if "abnormal" in original_class_name:
                        is_abnormal = True
                    
                    # (2) R값이 기준(0.88)보다 낮은지 확인 (낮으면 무조건 불량)
