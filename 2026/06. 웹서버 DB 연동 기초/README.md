...
> **이 폴더는 성격이 다릅니다.** 다른 폴더는 이미 만든 프로젝트를 설명한 것이고,
> 여기는 공부하려고 **새로 만든 실습**입니다.

**한 줄 요약**: 방명록 하나를 **웹 서버 3종 × DB 2종** 조합으로 만들어 보면서
"웹 서버가 뭐고 DB가 뭔지, 종류가 왜 나뉘고 뭘 골라야 하는지"를 익힙니다.

방법은 단순합니다. **똑같은 앱을 서버만 바꿔서, 그리고 DB만 바꿔서** 3벌
만들었습니다. 나머지를 고정해 놓고 한 축만 건드리면 **그 축이 뭘 하는 놈인지**
드러납니다.
...
> `python`이 아니라 **`py`** 를 씁니다. 이 PC의 `python`은 Microsoft Store 스텁이라
> 아무것도 하지 않습니다.
