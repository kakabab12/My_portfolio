...
| 기능 | 구현 내용 |
|---|---|
| 🖐️ **손 제스처 조종** | 웹캠 → MediaPipe 손 인식 → 손끝 위치를 D-pad 방향으로 매핑 → `/cmd_vel` 발행 |
| 🧤 **자이로 장갑 조종** | ESP32 + MPU6050 장갑 펌웨어를 **직접 작성**, Wi-Fi로 기울기 전송 |
| 🕹️ **조이스틱 조종** | 여러 입력 중 **하나만 로봇에 전달하는 mux 노드** 설계 |
...
| 폴더 | 내용 |
|---|---|
| [**`로봇 프로젝트 본체/`**](로봇%20프로젝트%20본체/) | ⭐ **실제 로봇 프로젝트.** 제스처 파이프라인, ROS 2 노드, 펌웨어, 지도, 실행 스크립트, 단위 테스트, 작업 기록. **상세 설명은 그 안의 README 참고** |
...
제어 컴퓨터를 Jetson Orin Nano로 옮기며 **6가지 문제가 겹쳐서** 발생했습니다.
...
최종적으로 OpenCR · LDS-03 라이다 · 조이스틱 · SLAM/Nav2가 **모두 정상 동작**하는 상태로 마무리했습니다.
당시 원본 로그와 수정된 설정 파일도 함께 보관되어 있습니다.
...
> ℹ️ `turtlebot3`, `DynamixelSDK`, `ld08_driver`, `coin_d4_driver` 등은
