...
| 폴더 | 내용 |
|---|---|
| [**`로봇 프로젝트 본체/`**](로봇%20프로젝트%20본체/) | ★ **실제 로봇 프로젝트.** 제스처 파이프라인, ROS 2 노드, 펌웨어, 지도, 실행 스크립트, 단위 테스트, 작업 기록이 들어 있다. **상세 설명은 그 안의 README 참고** |
...
로봇이 없어도 로직을 검증할 수 있게 **GPIO·하드웨어 접근부와 판단 로직을 분리**했다.
...
제어 컴퓨터를 Jetson Orin Nano로 옮기며 **6가지 문제가 겹쳐서** 터졌다.
각각을 **원인 → 조사 과정 → 수정 방법** 순으로 기록했다.
...
상태로 마무리했다. 당시 원본 로그와 수정된 설정 파일도 함께 보관되어 있다.
