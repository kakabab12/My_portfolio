...
void TurtleBot3::run()
{
  ...
  cmd_vel_callback();
  external_led_callback();
}

void TurtleBot3::external_led_callback()
{
  external_led_sub_ = this->create_subscription<std_msgs::msg::UInt8>(
    "opencr_led_status",
    rclcpp::QoS(rclcpp::KeepLast(10)),
    [this](const std_msgs::msg::UInt8::SharedPtr msg) -> void
    {
      if (msg->data > 6) {
        RCLCPP_WARN(this->get_logger(), "Ignoring invalid OpenCR LED mode: %u", msg->data);
        return;
      }
      ...
      dxl_sdk_wrapper_->set_data_to_device(
        extern_control_table.external_led_mode.addr,
        extern_control_table.external_led_mode.length,
        &mode,
        &sdk_msg);

      RCLCPP_INFO(
        this->get_logger(), "OpenCR LED mode: %u, sdk_msg: %s", mode, sdk_msg.c_str());
    });
}
