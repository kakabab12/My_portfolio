...
import anthropic
import torch
import pyrealsense2 as rs  # 리얼센스용 라이브러리 추가
...
# 카메라 세팅 (GStreamer 제거, USB 웹캠 및 리얼센스로 완전 전환)
CAM2_INDEX     = 7  # 다이소 USB 웹캠 주소 (v4l2-ctl 확인 결과)
...
model2 = load_model(MODEL_CAM2, "CAM2(USB Webcam)")
    ...
    if cam_id == 1: # CAM1 (숫자 인식부)
        ...
        if weight is not None:
            ...
            if over and system_running and (now - last_cmd_t1 > COMMAND_COOLDOWN):
                # 부저 울림(하드웨어 제어) 부분 완전히 삭제됨. 화면 및 웹 로그상 기록만 남김.
                action_log_1.append({"action": f"초과 중량 감지 ({weight}g)", "result": "기록됨"})
                if len(action_log_1) > 200: action_log_1.pop(0)
                last_cmd_t1 = now

    else: # CAM2 (박스 인식 및 로봇팔 제어부)
...
def capture1_loop():
    ...
    try:
        ...
        print(f"[ERROR] RealSense 초기화 실패: {e}")
        return
...
def capture2_loop():
    ...
    # CAM2: 다이소 USB 웹캠 V4L2 포맷 강제 지정 호출
    cap = cv2.VideoCapture(CAM2_INDEX, cv2.CAP_V4L2)
    if not cap.isOpened(): 
        print(f"[ERROR] USB 웹캠(index {CAM2_INDEX})을 열 수 없습니다.")
        return
