...
카메라 2대를 동시에 돌려 **박스를 분류**하고 **무게 숫자를 읽습니다.** 결과는
아두이노로 보내 **로봇팔을 움직입니다.**
...
원래는 **CSI 카메라(IMX219)** 로 가려고 GStreamer 파이프라인까지 다 짜 뒀습니다.
그런데 전시를 앞두고 **CSI 포트가 물리적으로 고장** 났습니다.

Intel RealSense와 일반 USB 웹캠 조합으로 **구조를 갈아엎었습니다.** FastAPI
멀티스레딩으로 두 카메라의 YOLO 추론을 병렬로 돌리게 다시 짜서 **전시회는
예정대로 마쳤습니다.** 바꾸기 전 CSI 버전 코드도 지우지 않고 같이 뒀습니다.
...
| 파일 | 설명 |
|---|---|
| **`project/index.html`** | 실시간 관제 UI. 듀얼 카메라를 동시에 스트리밍하고 CPU·FPS를 모니터링합니다. **챗봇으로 실시간 현장 데이터를 물어** 공정 상태를 확인할 수 있습니다 |
