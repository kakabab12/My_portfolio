...
FEATURE_KR = {
    ...
    'icd10_pcs':'수술 종류 코드','diag_count':'기저질환 수','surgery_rank':'수술 순번',
    ...
    'mean_nibp_sbp':'수축기 혈압 (평균)','std_nibp_sbp':'수축기 혈압 변동성',
    'min_nibp_sbp':'수축기 혈압 최저','max_nibp_sbp':'수축기 혈압 최고',
    'mean_nibp_dbp':'이완기 혈압 (평균)','std_nibp_dbp':'이완기 혈압 변동성',
    'mean_nibp_mbp':'평균동맥압',
...
def fmt_val(feat, val, raw_row=None):
    """숫자 인코딩 값을 사람이 읽을 수 있는 값으로 변환"""
    val = float(val)

    # 범주형 피처: 인코딩 숫자 → 원래 문자열
    if feat == 'department':
        orig = inverse_encoders.get('department', {}).get(int(val), str(int(val)))
        dept_kr = {'GS':'일반외과','CTS':'흉부외과','NS':'신경외과','OS':'정형외과',
                   'OG':'산부인과','UR':'비뇨기과','PS':'성형외과','OL':'안과','OT':'안과/기타'}
    ...
    if feat == 'asa':  return 'ASA ' + str(int(val)) + '등급'
...
def clinical_calibrate(base_prob, d, asa_val):
    ...
    inr = d.get('lab_inr')
    if inr and float(inr) > 2.0: boost += 0.3

    # 혈압 이상 보정
    sbp = d.get('lab_sbp')
    if sbp:
        s = float(sbp)
        if s > 180: boost += 0.35   # 고혈압 위기
        elif s > 160: boost += 0.15
        elif s < 90: boost += 0.5   # 저혈압 위험
...
def get_surgeries():
    ...
    if dept:
        ...
        if filtered.empty:
            filtered = pcs_lookup[pcs_lookup['department_name'].str.upper() == dept.upper()]
    else:
        filtered = pcs_lookup

    print(f"  결과: {len(filtered)}건")
    if filtered.empty:
        print(f"  가능한 dept: {pcs_lookup['department_name'].unique().tolist()}")
        return jsonify([])

    return jsonify([{
        'code':  str(r['icd10_pcs_code']),
        'label': str(r['icd10_pcs_code']) + '  (평균 ' + str(int(r['avg_duration'])) + '분 · ' + str(int(r['count'])) + '건)',
...
def predict():
    ...
    # 혈압을 생체징후 피처에 매핑
    bp_feat_sbp = [f for f in features if 'nibp_sbp' in f]
    bp_feat_dbp = [f for f in features if 'nibp_dbp' in f]
    lab_feature_map_ext['lab_sbp'] = bp_feat_sbp
    lab_feature_map_ext['lab_dbp'] = bp_feat_dbp

    lab_inputs_used = []
    for ik in ['lab_creatinine','lab_glucose','lab_hb','lab_albumin','lab_wbc',
               'lab_plt','lab_na','lab_k','lab_alt','lab_ast','lab_bun','lab_crp',
               'lab_inr','lab_tbili','lab_hct','lab_calcium','lab_sbp','lab_dbp']:
        val = d.get(ik)
    ...
    for col in ['diag_circulatory','diag_neoplasm','diag_endocrine','diag_respiratory',
        ...
        if d.get(col, 0): user_feats.add(col)
    ...
    def get_top_factors(imp, n=7):
        ...
        for i in idx:
            f = features[i]
            if f in seen_f or f == 'race': continue
            ik = feat_to_ik.get(f)
            if ik and ik in seen_ik: continue
            if len(out) >= n: break
            out.append(i); seen_f.add(f)
    ...
    def make_factors(imp, p80, p50):
        ...
        } for i in get_top_factors(imp) if feat_to_kr(features[i]) != '']

    risk_level = '고위험' if icu_prob >= 50 else ('중위험' if icu_prob >= 20 else '저위험')
    rec = (
        '해당 환자는 ICU 입원 확률 ' + str(icu_prob) + '%로 ' + risk_level + '군으로 분류됩니다. '
        '예상 수술 시간은 약 ' + str(round(surgery_min)) + '분이며 '
        '빠르면 ' + str(round(surgery_fast)) + '분, 늦으면 ' + str(round(surgery_slow)) + '분 예상됩니다. '
        + ('▶ 수술 전 ICU 사전 배정 및 집중치료팀 준비를 강력 권고합니다.' if icu_prob >= 50
           else '▶ 표준 수술 프로토콜 적용, 활력징후 모니터링을 권고합니다.' if icu_prob >= 20
           else '▶ 일반 회복실 배정 후 표준 경과 관찰이 적합합니다.')
    )

    return jsonify({
        'icu_prob': icu_prob, 'raw_prob': round(raw_prob, 1),
        'surgery_min': round(surgery_min),
        'surgery_fast': round(surgery_fast), 'surgery_slow': round(surgery_slow),
