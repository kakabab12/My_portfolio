...
class PoseEstimator:
    ...
    def __init__(self, config):
        ...
        # 워밍업 겸 GPU 실동작 검증. onnxruntime은 "CUDAExecutionProvider가 설치돼 있음"과
        # "이 GPU에서 실제로 커널을 실행할 수 있음"을 구분하지 않는다 — 세션 생성은 항상
        # 성공하고, 실패는 첫 추론 호출에서만 드러난다. 오래된 GPU(예: MX250·Pascal/Compute
        # Capability 6.1)는 최신 onnxruntime-gpu 배포판에 그 아키텍처용 커널이 없어 "no kernel
        # image available"로 죽는다(2026-07-15 실측) — _resolve_device의 "auto"는 provider
        # 목록만 보므로 이 실패를 못 걸러낸다. 여기서 실제로 한 번 돌려보고 실패하면 CPU로
        # 강등한다 — 오래된 GPU가 꽂힌 다른 배포 PC에서도 person_lock이 조용히 죽는 걸 막는다
        dummy = np.zeros((480, 640, 3), dtype=np.uint8)
        try:
            self.infer(dummy)
        except Exception:
            if device == "cpu":
                raise  # CPU조차 실패하면 원인이 다른 곳(모델 파일 등) — 그대로 올린다
            logger.warning(
                "포즈 GPU(%s) 추론 실패 — 이 GPU가 onnxruntime-gpu 커널을 지원하지 않는 것으로 "
                "보입니다. CPU로 강등합니다", device, exc_info=True,
            )
            device = "cpu"
            self._body = Body(mode=model["pose_mode"], backend="onnxruntime", device=device)
            self.infer(dummy)

        self.device = device  # 실제로 검증된 최종 장치 — smoke_test.py 등 외부에서 참고
