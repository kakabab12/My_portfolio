...
scripts/dpad_ui_demo.py에서 테스트로 다듬어진 손동작 D-pad 프로토타입을
main.py와 같은 격의 **공식 진입점**으로 승격한다 — 손 모양(한 손가락/주먹/
손바닥)·각도 기반 방향 판정·오프셋 커서·정지 자동 재정렬 등 판정 로직은
그 파일에서 그대로 가져왔다(각 상수 옆 주석이 테스트 튜닝 이력을 그대로
...
# 손끝 One Euro 필터 — dpad_ui_demo.py 2026-08-04 테스트 조정값(1.5/1.0 -> 0.7/0.4)을
...
DEMO_EXTEND_RATIO = 0.80   # 본 엔진 config.yaml 기본값(1.0)보다 낮춤 — 2026-08-04
                           ...
                           # 카메라 거리·자세 기준으로 이미 테스트 보정된 값이라 — 여기 값만
                           # 이 데모 전용)
