...
여기서 나오는 값은 전부 **가상 카메라 시뮬레이션**이다. 실제 장치에서 측정한
값은 `01_테스트_측정치.md`에 따로 모아 뒀다. 둘을 섞어 쓰면 안 된다.
...
#
# r을 훑으면서 네 가지를 측정한다. 논문 초안에서 "훑지 않았다"고 한계로 적었던
# 바로 그 곡선이다.
# ─────────────────────────────────────────────────────────────────────────
...
def sweep_blend_ratio():
    ...
    print("  ※ 입 벌림 간섭은 가상 카메라가 코 변형을 모형에 안 넣어 여기서 못 측정한다 —")
    print("     테스트 측정이 필요하다.")
    return rows
