...
:: 실행 중에는 콘솔에 cam on / cam off를 입력해 창을 켜고 끌 수 있다
```
...
| `py scripts\calibrate.py` | 임계값 자동 보정 — 실제 동작을 측정해 config에 반영 |
...
| 실행 중 `tune` (+Enter) | **실시간 감도 조절 슬라이더** — "고개를 몇 도 돌리면 화면 끝인가"(가로·세로 각도). 창을 닫아도 값이 유지된다. 2026-09-24에 효과가 없던 감도·곡률 슬라이더 세 개를 뺐다 |
...
- 배럴 왜곡 되돌리기는 **기본 꺼짐** — 얼굴이 정규 모형과 다르면 해로운데
  그것을 가려낼 잡음-독립적 지표를 찾지 못했다(2026-09-24에 설정 항목째 삭제)
...
(좌우 7도면 화면 폭의 45.8%까지). 그 사람의 실제 가동 범위를 측정해 권장값을
...
캔버스는 실제 해상도를, 화면 끝을 보는 고개 각도는 EDID 실치수를, 랜드마크 척도는 두 눈
...
이 판정은 테스트에서 두 번 어긋났다. 두 번 모두 짐작으로 고쳤는데, 알고 보니
두 번째 문제는 첫 번째 수정 때문에 생긴 것이었다. 그래서 판정을 모듈로 꺼내고
...
찾아보니, 렌즈 자가 보정 한 번(`cv2.calibrateCamera`)이 4.10에서 수십 초 걸렸다
...
| 머리 자세 각도 매핑 (`HEAD_POSE_MAPPING`) | 한 번도 켜서 쓴 적이 없다. 상대 회전 매핑이 같은 문제를 푼다 |
...
- **라이선스 (2026-08-27 갱신)**: 스택 전체가 상업 사용 가능 + 코드 공개(카피레프트) 의무 없음 —
  MediaPipe(Apache-2.0) + Pillow(MIT-CMU). 손 모양 판별은 자체 기하 규칙이라 추가 의존성이 없다.
  Apache·MIT 모두 배포물에 라이선스 문서를 함께 넣는 것이 통상 절차이고, 제품 화면에 표시할 의무는 없다.
