...
보정을 부르며 테스트 버그의 주 원인이 됐다(획 씹힘·배구 토스 — 당일 테스트 5건).
...
REENTRY_SPAN_RATIO = 4.0      # 소실 재등장 이음 반경(2026-07-31 키오스크 테스트 — 획 중간
...
class HandSelector:
    ...
    def __init__(self, config, frame_width_px, frame_height_px, clock=time.monotonic):
        ...
        # 어깨선 추정 계수(2026-07-31 키오스크 테스트) — None(키 없음)이면 종전(어깨선
    ...
    def _acquirable_heads(self, heads):
        ...
        2026-08-04 테스트 보완: 앵커가 없을 때만 쓰인다(_update_head_anchor) —
    ...
    def shoulder_line_y_ratio(self):
        """어깨선 추정 — 앵커(머리) 기준 몸 비례로 복원 (2026-07-31 키오스크 테스트).

        휴식 존이 화면 하단 띠(절대 좌표)뿐이면 카메라 각도·거치에 따라 가슴
        높이 손이 띠에 걸려 위 쓸기 게이트가 과잉 발동한다(테스트: 위만 안 잡힘).
