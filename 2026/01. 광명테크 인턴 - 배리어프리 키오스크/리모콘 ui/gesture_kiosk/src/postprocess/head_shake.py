...
검출 순간 끊김 유예(dropout_grace_sec, 2026-08-05 사용자 보고 — "키오스크
가까이서 흔들어야 잘되고 멀리서는 잘 안 된다"): 종전에는 nose_px가 단 한
프레임이라도 None이면(검출 실패) 그 즉시 누적(_extreme_x·_direction·
_reversal_sec_list)을 전부 버렸다. 얼굴이 멀어질수록 화면에서 차지하는
픽셀이 줄어 프레임마다 검출 신뢰도가 경계선을 넘나들며 흔들리기 쉬운데,
반전이 window_sec(1.5초) 안에 4회 쌓이기도 전에 한 프레임만 놓쳐도 처음부터
다시 시작해야 했던 것이 원거리 인식률 저하의 실질 원인으로 추정된다.
gesture_filter.py 스와이프 궤적의 dropout_grace_sec(모션 블러로 인한 순간
추적 끊김을 짧게 봐주는 것)와 같은 개념을 여기도 도입: 마지막으로 검출이
성공한 시각(_last_good_sec) 대비 dropout_grace_sec 안이면 이번 프레임만
건너뛰고 누적은 그대로 유지한다 — 그보다 오래 끊기면(정말로 얼굴이 사라진
경우) 그때 가서 리셋한다. 키 없으면(기본 0.0) 종전과 완전히 동일(유예 없이
즉시 리셋) — 회귀 방지(tests/test_head_shake.py의 test_face_loss_resets_progress
는 grace=0.0을 그대로 검증).
"""
import time
...
class HeadShakeDetector:
    ...
    def __init__(self, config, clock=time.monotonic):
        ...
        self._dropout_grace_sec = shake.get("dropout_grace_sec", 0.0)
        self._clock = clock
        self._last_fire_sec = None
        self.reset()

    def reset(self):
        """얼굴 소실(유예 초과)·모드 전환 직후 — 진행 중이던 왕복 누적을 버린다."""
        ...
        self._last_good_sec = None      # 마지막으로 검출이 성공한 시각 — dropout_grace_sec 기준
    ...
    def update(self, nose_px, interocular_dist_px):
        ...
        nose_px가 None(얼굴 미검출)이면 원칙적으로 누적을 버린다 — 끊긴 궤적을
        이어 붙이면 얼굴이 사라졌다 나타나는 것만으로 반전이 잡힐 수 있다. 다만
        직전 검출 성공 이후 dropout_grace_sec 안이면 순간적인 검출 실패로 보고
        이번 프레임만 건너뛴다(누적 유지) — 모듈독스트링의 "검출 순간 끊김
        유예" 참고.
        """
        now_sec = self._clock()
        if nose_px is None or interocular_dist_px < MIN_INTEROCULAR_DIST_PX:
            if (self._last_good_sec is not None
                    and now_sec - self._last_good_sec < self._dropout_grace_sec):
                return False   # 짧은 유예 안 — 누적은 그대로 두고 이번 프레임만 무시
            self.reset()
            return False
        self._last_good_sec = now_sec
