...
행렬**을 직접 구한다. 그러면
  ...
  · 측정할 것이 남지 않는다. 감도는 "고개를 몇 도 돌리면 화면 끝인가"라는
...
★반사 켤레와 부호 자가 학습 (2026-08-31 저녁 — 테스트 보고 "좌우가 반대")
------------------------------------------------------------------------
...
가상 카메라로 측정한 "고개 pitch 대 코의 세로 상대이동"이 그것을 보여 준다:
...
#
# 크기는 프로베니우스 노름(중심을 뺀 좌표 제곱합의 제곱근)으로 측정한다.
...
def _orthonormal_frame(points):
    ...
    **부호를 왜 측정할 필요가 없는가**: 이 축들은 랜드마크에서 곧바로 나오고,
...
class HeadOrientation:
    ...
    def __init__(self, rotation_source="auto", lens=None):
        ...
        배치 9종으로도 측정해 봤는데 **전 배치에서 랜드마크가 이겼다** (최악은
    ...
    def _decide_signs(self, axes):
        ...
        ★진실 기준은 "화면에서 보이는 이동 방향"이다 — 사용자의 머리 방향이
        아니다. 그 둘은 프레임이 거울일 때만 일치한다. 거울 기준을 쓰는 이유와
        그것을 두 번 갈아엎은 경위는 tests/test_virtual_camera_matrix.py의
        모듈 독스트링에 있다. 배포 기본은 camera.mirror=true라 두 기준이
        일치하며, 이 값이 실제 카메라 영상과 어긋나면(예: 카메라가 이미
        좌우 반전된 영상을 내보내는 기종) 좌우가 반대로 느껴진다 — 그때는
        부호 로직이 아니라 camera.mirror를 실제 카메라에 맞춰야 한다.
        """
        x_axis = axes[0]
        self._sign_h = 1.0
