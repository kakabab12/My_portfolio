...
광각 렌즈의 진짜 문제는 **배럴 왜곡**이다. 가상 카메라에 왜곡을 넣어 측정해 보니
...
즉 중심을 뺀 좌표에 (1 + z/f)를 곱하면 원근이 풀린다. 가상 카메라로 측정한
...
얼굴은 제각각이라, 눈이 넓거나 코가 낮으면 **치수가 틀린 자로 측정하는 셈**이다.

가상 카메라에 인체 계측 범위의 얼굴 10종을 넣어 측정해 봤다(광각 90도, 참 f=430).
그랬더니 두 가지가 드러났다.
...
그래서 **2단으로 채택한다.**
  ...
  2단 (왜곡까지) — k1까지 되돌리는 단계. 2026-09-24 삭제했다(아래 표).
...
그리고 **왜곡 되돌리기(k1)는 기본으로 끈다.** 측정해 보니 이렇다.
...
안전했다. 그래서 왜곡 되돌리기는 2026-09-24 선택지째 뺐다 — 꺼 둔 채 한 번도
쓰지 않았고, 켜면 해로울 수 있는데 켤지 가려낼 방법이 없었다.
...
머물면 왜곡의 증거가 없고, 그때 나오는 값은 부호까지 틀린다. 실제로 측정했다
...
그래서 다음을 모두 통과해야 채택한다.
  ...
  3) **교차검증** — 뷰를 반으로 갈라 따로 보정했을 때 두 f가 FOCAL_CROSS_TOL
     안에서 일치한다. 위 표(k1로 잰 것)처럼 이 하나가 못 믿을 경우를 걸러낸다
  4) 재투영 오차(RMS)가 MAX_RMS_PX 이하다
  5) f가 물리적으로 말이 되는 범위다
...
부하를 측정해 보니 최대 멈춤이 6.78ms에서 109ms로 뛰었다 — CPU를 나눠 쓰기
...
보정 한 번은 60뷰 기준 60~81ms이고(OpenCV 5.0. 4.10에서는 같은 계산이 수십 초라
2026-09-25 고정 버전을 5.0으로 올렸다 — requirements.txt), 교차검증까지 세 번 부르므로 한 덩어리로
두면 200ms 가까이 CPU를 붙잡는다. 사이사이 짧게 쉬어(_BREATH_SEC) 그 덩어리를
쪼갠다 — 총 시간은 그대로지만 커서가 한 번에 오래 멈추지 않는다.

(cv2.setNumThreads(1)로 OpenCV를 단일 스레드로 돌리는 것도 측정해 봤는데 오히려
    ...
    def _solve(self, views):
        ...
        try:
            ...
            if focal_gap > FOCAL_CROSS_TOL:
                ...
                return
            # 왜곡 계수(k1)는 f와 함께 맞추기만 하고 버린다 — 원근(f)만 되돌린다.
            # 맞추는 방식은 그대로 둔다(바꾸면 f 추정값도 달라진다). k1까지 되돌리는
            # 선택지는 2026-09-24 삭제했다 — 위 독스트링의 표 참고
            k1 = 0.0
            with self._lock:
...
def _lower_priority():
    ...
    6.78ms에서 109ms로 뛰는 것을 측정했다. 보정은 몇 초 늦어도 상관없다.
    """
    try:
...
def _calibrate(views, width, height):
    ...
    **2모수(f, k1)만 푼다.** k2까지 풀면 둘이 서로를 상쇄해 오히려 나빠진다 —
    가상 카메라로 측정했다 (참 k1=-0.30일 때 2모수 -0.284, 3모수 -0.236).
    """
