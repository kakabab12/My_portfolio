...
numpy 1.26.4)은 만일의 통합 대비 보험이었다(numpy·OpenCV는 2026-09-25에
되올림 — requirements.txt 주석). 이 파일은 의존이 설치된
...
def disable_console_quick_edit():
    """콘솔 빠른 편집(QuickEdit) 해제 — 터치 먹통 방지 (2026-07-31 키오스크 테스트).

    윈도우 콘솔은 클릭/터치로 "선택 모드"에 들어가면 프로세스 출력이 통째로
    정지한다 — stdout이 이벤트 채널인 이 엔진은 그대로 멈춘 것처럼 보였다
    (테스트: 제목줄 "선택 관리자" 먹통). 터치스크린 키오스크에서는 콘솔을
