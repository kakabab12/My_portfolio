    ...
    return {
        "mode_switch": {
            "head_shake": {
                ...
                "dropout_grace_sec": dropout_grace_sec,
            }
        }
    }
...
class DropoutGraceTest(unittest.TestCase):
    """검출 순간 끊김 유예(2026-08-05) — 원거리에서 얼굴 검출이 프레임 단위로
    깜빡여도(1~2프레임 미검출) 진행 중인 왕복 누적이 살아남는지 검증."""

    def test_brief_dropout_within_grace_preserves_progress(self):
        clock = FakeClock()
        detector = HeadShakeDetector(make_config(dropout_grace_sec=0.3), clock=clock)
        detector.update((0.0, 0.0), EYE_DIST_PX)
        clock.tick(0.03)
        detector.update((0.4 * EYE_DIST_PX, 0.0), EYE_DIST_PX)   # 첫 획
        clock.tick(0.03)
        detector.update((0.0, 0.0), EYE_DIST_PX)                  # 반전 1 — 진행 중
        self.assertGreater(detector.progress_ratio, 0.0)
        progress_before = detector.progress_ratio

        clock.tick(0.1)   # 짧은 검출 끊김 — grace(0.3) 이내
        detector.update(None, 0.0)
        self.assertEqual(detector.progress_ratio, progress_before)   # 누적 유지(리셋 안 됨)

        clock.tick(0.03)   # 검출 재개, 왕복 이어가기
        detector.update((0.4 * EYE_DIST_PX, 0.0), EYE_DIST_PX)   # 반전 2
        clock.tick(0.03)
        result = detector.update((0.0, 0.0), EYE_DIST_PX)         # 반전 3 — 확정(min_reversals=3)
        self.assertTrue(result)

    def test_dropout_longer_than_grace_still_resets(self):
        clock = FakeClock()
        detector = HeadShakeDetector(make_config(dropout_grace_sec=0.3), clock=clock)
        detector.update((0.0, 0.0), EYE_DIST_PX)
        clock.tick(0.03)
        detector.update((0.4 * EYE_DIST_PX, 0.0), EYE_DIST_PX)
        clock.tick(0.03)
        detector.update((0.0, 0.0), EYE_DIST_PX)   # 반전 1 — 진행 중
        self.assertGreater(detector.progress_ratio, 0.0)

        clock.tick(1.0)   # 긴 끊김 — grace(0.3) 초과, 정말로 얼굴이 사라진 경우로 취급
        detector.update(None, 0.0)
        self.assertEqual(detector.progress_ratio, 0.0)   # 리셋됨

    def test_zero_grace_keeps_old_immediate_reset_behavior(self):
        # dropout_grace_sec 기본값(0.0) — 유예 없이 종전대로 즉시 리셋
        clock = FakeClock()
        detector = HeadShakeDetector(make_config(), clock=clock)   # 기본 0.0
        detector.update((0.0, 0.0), EYE_DIST_PX)
        clock.tick(0.03)
        ...
        clock.tick(0.01)   # 아주 짧은 끊김이라도 grace=0.0이면 그대로 리셋
        detector.update(None, 0.0)
        self.assertEqual(detector.progress_ratio, 0.0)
