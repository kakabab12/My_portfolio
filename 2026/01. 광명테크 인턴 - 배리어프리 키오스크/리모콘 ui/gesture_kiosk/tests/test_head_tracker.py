"""head_tracker 단위 테스트 — 카메라·mediapipe 없이 판정 로직만 검증한다.

구 프로토타입(헤드트래커_프로토타입_win.ver) 테스트를 이식하되(2026-07-30 병합),
아이트래커(시선) 모드·person_lock 세대(generation) 개념은 제외했다 — 이 판은
select_user_face(크기 기준, 세대 없음)로 사용자를 고른다.

실행 (프로젝트 루트에서):
    python -m unittest discover tests -v
"""
import os
import sys
import unittest

sys.path.insert(0, os.path.dirname(os.path.dirname(os.path.abspath(__file__))))

import numpy as np

from src.inference.face_estimator import (
    LMK_LEFT_EYE_OUTER, LMK_NOSE_TIP, LMK_RIGHT_EYE_OUTER, FaceLandmarks,
)
from src.postprocess.head_tracker import HeadTracker

NUM_LANDMARKS = 478
FRAME_DT_SEC = 1.0 / 30.0


class FakeClock:
    def __init__(self):
        self.now_sec = 1000.0

    def __call__(self):
        return self.now_sec

    def tick(self, dt_sec):
        self.now_sec += dt_sec


def make_face(nose_px=(100.0, 100.0), eye_dist_px=40.0, jaw_open=0.0,
             eye_close=0.0, eye_close_left=None, eye_close_right=None, mouth_pucker=0.0,
             nose_local_px=(0.0, 0.0)):
    """FaceLandmarks 테스트 대역 — mediapipe 임포트 없이 필요한 신호만 채운다.

    eye_close는 양쪽에 같은 값을 채우는 편의 인자 — 좌우를 다르게(윙크) 테스트할
    때만 eye_close_left/right를 개별로 지정한다.

    ★2026-08-20 — 두 인자의 의미가 확실히 갈린다(_CursorMapper 재설계 대응):

      nose_px       : 얼굴 전체가 화면 어디에 있는지 = **몸이 움직인 것**(평행이동).
                      눈도 코를 따라 함께 옮겨간다. 커서는 움직이면 **안 된다**.
      nose_local_px : 두 눈 중점을 기준으로 코가 얼마나 치우쳤는지 = **고개를 돌린 것**.
                      눈은 그대로 두고 코만 옮긴다. 커서는 이만큼 움직여야 한다.

    예전 대역은 nose_px 하나뿐이었고 눈이 코를 따라다녔다 — 즉 "얼굴을 통째로
    옆으로 옮겨 놓고 커서가 움직이길 기대"하고 있었다. 그게 실사용에서 "가만히
    있어도 커서가 밀려간다"로 나타났던 바로 그 동작이다.
    """
    landmarks_px = np.zeros((NUM_LANDMARKS, 2), dtype=np.float32)
    eye_mid_x, eye_mid_y = nose_px
    landmarks_px[LMK_NOSE_TIP] = (eye_mid_x + nose_local_px[0], eye_mid_y + nose_local_px[1])
    landmarks_px[LMK_LEFT_EYE_OUTER] = (eye_mid_x - eye_dist_px / 2.0, eye_mid_y)
    landmarks_px[LMK_RIGHT_EYE_OUTER] = (eye_mid_x + eye_dist_px / 2.0, eye_mid_y)
    blendshapes = {
        "jawOpen": jaw_open,
        "eyeBlinkLeft": eye_close if eye_close_left is None else eye_close_left,
        "eyeBlinkRight": eye_close if eye_close_right is None else eye_close_right,
        "mouthPucker": mouth_pucker,
    }
    return FaceLandmarks(bbox=(0, 0, 200, 200), conf=1.0, landmarks_px=landmarks_px,
                         blendshapes=blendshapes)


def make_config(smoothing_alpha=1.0, distance_smoothing_alpha=1.0, dwell_enabled=True,
               recenter_enabled=True, recenter_dwell_enabled=False, recenter_dwell_sec=0.3):
    return {
        "head_tracker": {
            "calibration_window_sec": 0.1,
            "pointer": {
                "sensitivity_x": 2.0,
                "sensitivity_y": 2.0,
                "smoothing_alpha": smoothing_alpha,
                "distance_smoothing_alpha": distance_smoothing_alpha,
                "max_offset_ratio": 0.5,
            },
            "click": {"min_interval_sec": 0.2},
            "mouth_click": {"enabled": True, "open_margin": 0.5, "close_margin": 0.3},
            "eye_close_home": {"close_margin": 0.5, "hold_sec": 0.3},
            "recenter_gesture": {
                "enabled": recenter_enabled, "open_margin": 0.5, "close_margin": 0.3,
                "cooldown_sec": 0.2,
            },
            "dwell_click": {
                "enabled": dwell_enabled, "radius_ratio": 0.05, "dwell_sec": 0.3,
                "require_release_to_rearm": True,
            },
            "recenter_dwell": {
                "enabled": recenter_dwell_enabled, "radius_ratio": 0.05,
                "dwell_sec": recenter_dwell_sec, "require_release_to_rearm": True,
            },
        }
    }


class HeadTrackerTestBase(unittest.TestCase):
    def setUp(self, **config_kwargs):
        self.clock = FakeClock()
        self.tracker = HeadTracker(make_config(**config_kwargs), clock=self.clock)

    def _settle_calibration(self, nose_px=(100.0, 100.0), jaw_open=0.0, eye_close=0.0,
                           max_frames=20):
        """커서가 확정될 때까지만 같은 위치를 공급하고 즉시 돌려준다.

        완료 직후 추가로 프레임을 더 태우면 dwell 타이머가 그만큼 미리 누적돼(커서가
        캘리브레이션 완료 순간부터 바로 dwell 추적을 시작하므로) 이후 타이밍 assert가
        어긋난다 — 그래서 캘리브레이션이 끝나는 정확한 프레임에서 멈춘다.
        """
        for _ in range(max_frames):
            face = make_face(nose_px=nose_px, jaw_open=jaw_open, eye_close=eye_close)
            result = self.tracker.update(face)
            if result.cursor_x_ratio is not None:
                return result
            self.clock.tick(FRAME_DT_SEC)
        raise AssertionError("캘리브레이션이 max_frames 안에 끝나지 않았다")


class CursorMappingTest(HeadTrackerTestBase):
    def test_calibration_centers_cursor(self):
        result = self._settle_calibration()
        self.assertAlmostEqual(result.cursor_x_ratio, 0.5, places=2)
        self.assertAlmostEqual(result.cursor_y_ratio, 0.5, places=2)
        self.assertTrue(result.is_tracking)

    def test_cursor_moves_when_head_turns(self):
        """고개를 돌리면(눈은 그대로, 코만 옆으로) 커서가 그만큼 움직인다."""
        self._settle_calibration(nose_px=(100.0, 100.0))
        # eye_dist=40px, 코가 얼굴 기준 2px 옆으로 -> 2/40 x gain(2.0) = 0.10
        # -> x sensitivity(2.0) = 0.20 -> 커서 0.5 + 0.20 = 0.70
        result = self.tracker.update(make_face(nose_local_px=(2.0, 0.0)))
        self.assertAlmostEqual(result.cursor_x_ratio, 0.70, places=2)
        self.assertAlmostEqual(result.cursor_y_ratio, 0.5, places=2)

    def test_cursor_clamps_at_screen_edge(self):
        """크게 돌려도 max_offset_ratio(0.5)를 넘지 않는다 — 화면 밖으로 안 나간다."""
        self._settle_calibration(nose_px=(100.0, 100.0))
        result = self.tracker.update(make_face(nose_local_px=(20.0, 0.0)))
        self.assertAlmostEqual(result.cursor_x_ratio, 1.0, places=2)

    def test_cursor_ignores_whole_face_translation(self):
        """★회귀 방지 — 얼굴이 통째로 옮겨가는 것(몸이 움직이거나 자세를 고쳐
        앉는 것)에는 커서가 반응하면 안 된다.

        실사용 최초 보고 "고개를 가만히 있어도 커서가 조금씩 움직인다"의 원인이
        정확히 이것이었다. 예전 매핑은 코의 화면상 절대 위치만 봤기 때문에
        얼굴이 통째로 옆으로 가면 고개를 돌린 것과 구분하지 못했고, 기준점이
        고정값이라 되돌아오지도 않아 계속 밀려갔다.
        """
        self._settle_calibration(nose_px=(100.0, 100.0))
        for moved_to in ((160.0, 100.0), (40.0, 100.0), (100.0, 220.0), (100.0, 20.0)):
            result = self.tracker.update(make_face(nose_px=moved_to))
            self.assertAlmostEqual(result.cursor_x_ratio, 0.5, places=2,
                                   msg=f"얼굴만 {moved_to}로 옮겼는데 커서가 가로로 움직였다")
            self.assertAlmostEqual(result.cursor_y_ratio, 0.5, places=2,
                                   msg=f"얼굴만 {moved_to}로 옮겼는데 커서가 세로로 움직였다")

    def test_cursor_ignores_camera_distance_change(self):
        """카메라에 가까워지거나 멀어져도(얼굴 크기 변화) 커서는 제자리."""
        self._settle_calibration(nose_px=(100.0, 100.0))
        for dist in (80.0, 20.0):   # 두 배 가까이 / 절반 멀리
            result = self.tracker.update(make_face(eye_dist_px=dist))
            self.assertAlmostEqual(result.cursor_x_ratio, 0.5, places=2,
                                   msg=f"눈 사이 거리 {dist}px에서 커서가 움직였다")

    def test_smoothing_lags_step_change(self):
        self.setUp(smoothing_alpha=0.5)
        self._settle_calibration(nose_px=(100.0, 100.0))
        result = self.tracker.update(make_face(nose_local_px=(2.0, 0.0)))
        # alpha=0.5 — 목표(0.70)까지 한 프레임 만에 못 간다 (0.5와 0.70 사이)
        self.assertGreater(result.cursor_x_ratio, 0.5)
        self.assertLess(result.cursor_x_ratio, 0.70)

    def test_calibration_median_resists_single_outlier_frame(self):
        # calibration_window_sec(0.1s) 동안 프레임 여러 장이 들어간다 — 그중 1장만
        # 크게 아래로 쏠린 값(과도기 프레임 모사)이어도 중앙값 기준점은 흔들리지 않아야 한다
        nose_local_ys = [0.0, 0.0, 0.0, 0.0, 200.0]
        result = None
        for nose_local_y in nose_local_ys:
            result = self.tracker.update(make_face(nose_local_px=(0.0, nose_local_y)))
            if result.cursor_x_ratio is not None:
                break
            self.clock.tick(FRAME_DT_SEC)
        self.assertIsNotNone(result.cursor_x_ratio)

        # 캘리브레이션 후 원래 위치(100,100)로 돌아오면 중앙값 기준점(100)과 거의 일치 —
        # 평균(150)이었다면 여기서 위로 크게 벗어난 것처럼 오판정됐을 것
        result = self.tracker.update(make_face(nose_px=(100.0, 100.0)))
        self.assertAlmostEqual(result.cursor_y_ratio, 0.5, delta=0.05)

    def test_brief_face_loss_keeps_calibration(self):
        """★한두 프레임 놓친 것으로는 캘리브레이션을 버리지 않는다 (2026-08-31).

        예전에는 face가 None인 즉시 전부 리셋했다. 얼굴이 잠깐씩 끊기는
        조건(기울여 단 카메라, 역광)에서는 그것이 리셋->재캘리브레이션의
        되풀이가 되어 커서가 영영 안 나왔다 — 실기 보고 "옆으로 살짝 기운
        카메라는 커서가 아예 안 움직인다".
        """
        self._settle_calibration(nose_px=(100.0, 100.0))
        result = self.tracker.update(None)
        self.assertIsNone(result.cursor_x_ratio)
        self.assertFalse(result.is_tracking)
        self.assertEqual(result.events, [])
        # 곧바로 다시 잡히면 캘리브레이션을 다시 하지 않는다
        result = self.tracker.update(make_face(nose_px=(100.0, 100.0)))
        self.assertIsNotNone(result.cursor_x_ratio)
        self.assertTrue(result.is_tracking)

    def test_sustained_face_loss_resets_calibration(self):
        """오래 안 보이면 새 사용자로 보고 처음부터 다시 잡는다."""
        from src.postprocess.head_tracker import FACE_LOST_RESET_SEC

        self._settle_calibration(nose_px=(100.0, 100.0))
        # 첫 미검출 프레임에서 "언제부터 안 보였는지" 시계가 시작된다
        self.tracker.update(None)
        self.clock.tick(FACE_LOST_RESET_SEC + 0.1)
        result = self.tracker.update(None)
        self.assertFalse(result.is_tracking)
        # 이번엔 리셋됐으므로 다시 캘리브레이션부터
        result = self.tracker.update(make_face(nose_px=(100.0, 100.0)))
        self.assertIsNone(result.cursor_x_ratio)


class MouthClickTest(HeadTrackerTestBase):
    def setUp(self):
        # dwell 비활성 — 코 위치가 고정된 이 테스트들에서 dwell도 같이 진행돼
        # 공용 클릭 쿨다운을 오염시키는 것을 막는다 (mouth 신호만 격리해서 본다)
        super().setUp(dwell_enabled=False)

    def test_mouth_open_fires_once_per_episode(self):
        self._settle_calibration()
        result = self.tracker.update(make_face(jaw_open=0.8))
        self.assertEqual(len(result.events), 1)
        self.assertEqual(result.events[0].class_name, "select")
        self.assertEqual(result.events[0].data["trigger"], "mouth")

        # 입을 계속 벌리고 있어도 재발화 없음
        result = self.tracker.update(make_face(jaw_open=0.8))
        self.assertEqual(len(result.events), 0)

    def test_mouth_click_respects_shared_cooldown(self):
        self._settle_calibration()
        self.tracker.update(make_face(jaw_open=0.8))          # 1차 확정
        self.tracker.update(make_face(jaw_open=0.1))          # 닫음 — 재장전
        self.clock.tick(0.05)
        result = self.tracker.update(make_face(jaw_open=0.8))  # min_interval_sec(0.2) 이내
        self.assertEqual(len(result.events), 0)

    def test_mouth_click_fires_again_after_cooldown(self):
        self._settle_calibration()
        self.tracker.update(make_face(jaw_open=0.8))
        self.tracker.update(make_face(jaw_open=0.1))
        self.clock.tick(0.3)
        self.tracker.update(make_face(jaw_open=0.1))
        result = self.tracker.update(make_face(jaw_open=0.8))
        self.assertEqual(len(result.events), 1)


class EyeCloseHomeTest(HeadTrackerTestBase):
    def setUp(self):
        super().setUp(dwell_enabled=False)   # 눈 감김 신호만 격리 (MouthClickTest와 같은 이유)

    def test_no_event_before_hold_duration(self):
        self._settle_calibration()
        self.tracker.update(make_face(eye_close=0.8))   # 감기 시작
        self.clock.tick(0.29)                             # hold_sec(0.3) 미만
        result = self.tracker.update(make_face(eye_close=0.8))
        self.assertEqual(len(result.events), 0)

    def test_fires_once_after_hold_duration(self):
        self._settle_calibration()
        self.tracker.update(make_face(eye_close=0.8))
        self.clock.tick(0.31)
        result = self.tracker.update(make_face(eye_close=0.8))
        self.assertEqual(len(result.events), 1)
        self.assertEqual(result.events[0].class_name, "home")
        self.assertEqual(result.events[0].data["trigger"], "eye_close")

    def test_does_not_refire_while_sustained(self):
        self._settle_calibration()
        self.tracker.update(make_face(eye_close=0.8))
        self.clock.tick(0.31)
        self.tracker.update(make_face(eye_close=0.8))          # 1차 확정
        result = self.tracker.update(make_face(eye_close=0.8))  # 계속 감은 채 — 재발화 없음
        self.assertEqual(len(result.events), 0)

    def test_refires_after_opening_and_reholding(self):
        self._settle_calibration()
        self.tracker.update(make_face(eye_close=0.8))
        self.clock.tick(0.31)
        self.tracker.update(make_face(eye_close=0.8))   # 1차 확정

        self.tracker.update(make_face(eye_close=0.0))    # 눈 뜸 — 재장전
        self.tracker.update(make_face(eye_close=0.8))    # 다시 감기 시작
        self.clock.tick(0.31)
        result = self.tracker.update(make_face(eye_close=0.8))
        self.assertEqual(len(result.events), 1)

    def test_one_eye_wink_does_not_fire(self):
        # 한쪽만 감으면(윙크) 안 걸린다 — 양쪽 다 감아야 인정(오발화 방지 + 윙크
        # 못 하는 사용자도 조작 가능하게)
        self._settle_calibration()
        for _ in range(20):
            result = self.tracker.update(
                make_face(eye_close_left=0.9, eye_close_right=0.1)
            )
            self.clock.tick(FRAME_DT_SEC)
        self.assertEqual(len(result.events), 0)


class RecenterGestureTest(HeadTrackerTestBase):
    def setUp(self):
        super().setUp(dwell_enabled=False)   # 입 오므림 신호만 격리 (MouthClickTest와 같은 이유)

    def test_mouth_pucker_fires_calibration_and_hides_cursor(self):
        self._settle_calibration()
        result = self.tracker.update(make_face(mouth_pucker=0.9))
        self.assertEqual(len(result.events), 1)
        self.assertEqual(result.events[0].class_name, "calibration")
        self.assertEqual(result.events[0].data["trigger"], "mouth_pucker")
        # 리셋은 다음 프레임부터 반영된다(이번 프레임 커서는 리셋 전에 이미 계산됨) —
        # 캘리브레이션이 다시 시작돼 다음 프레임은 커서가 미확정 상태다
        result = self.tracker.update(make_face(mouth_pucker=0.1))
        self.assertIsNone(result.cursor_x_ratio)

    def test_recenter_does_not_reset_mouth_baseline(self):
        self._settle_calibration()
        self.tracker.update(make_face(mouth_pucker=0.9))   # 재정렬 발화
        # jaw 기준선은 그대로라 재캘리브레이션 대기 없이 바로 입 벌리기가 먹힌다
        result = self.tracker.update(make_face(jaw_open=0.8))
        select_events = [e for e in result.events if e.class_name == "select"]
        self.assertEqual(len(select_events), 1)

    def test_recenter_respects_cooldown(self):
        self._settle_calibration()
        self.tracker.update(make_face(mouth_pucker=0.9))
        self.tracker.update(make_face(mouth_pucker=0.1))   # 재장전(히스테리시스 close_margin 아래)
        self.clock.tick(0.05)                                # cooldown_sec(0.2) 이내
        result = self.tracker.update(make_face(mouth_pucker=0.9))
        calib_events = [e for e in result.events if e.class_name == "calibration"]
        self.assertEqual(len(calib_events), 0)

    def test_disabled_recenter_never_fires(self):
        super().setUp(dwell_enabled=False, recenter_enabled=False)
        self._settle_calibration()
        result = self.tracker.update(make_face(mouth_pucker=0.9))
        self.assertEqual(result.events, [])


class RecenterDwellTest(HeadTrackerTestBase):
    """응시 -> calibration (2026-07-30 신설 — 입 오므리기 오탐 대체)."""

    def setUp(self):
        # select용 dwell(0.3초)을 꺼서 recenter_dwell(같은 커서 신호) 신호만 격리해서 본다
        super().setUp(dwell_enabled=False, recenter_dwell_enabled=True, recenter_dwell_sec=0.3)

    def test_dwell_fires_calibration_and_resets_cursor(self):
        self._settle_calibration()
        self.tracker.update(make_face())          # 응시 시작(앵커 설정)
        self.clock.tick(0.31)
        result = self.tracker.update(make_face())
        self.assertEqual(len(result.events), 1)
        self.assertEqual(result.events[0].class_name, "calibration")
        self.assertEqual(result.events[0].data["trigger"], "dwell")
        # 재정렬로 캘리브레이션이 다시 시작돼 다음 프레임은 커서가 미확정 상태다
        result = self.tracker.update(make_face())
        self.assertIsNone(result.cursor_x_ratio)

    def test_no_event_before_dwell_duration(self):
        self._settle_calibration()
        self.tracker.update(make_face())
        self.clock.tick(0.29)
        result = self.tracker.update(make_face())
        self.assertEqual(len(result.events), 0)

    def test_moving_away_resets_progress(self):
        self._settle_calibration()
        self.tracker.update(make_face())
        self.clock.tick(0.2)
        moved = make_face(nose_local_px=(20.0, 0.0))   # 고개를 크게 돌림 — 반경 이탈로 앵커 재설정
        self.tracker.update(moved)
        self.clock.tick(0.2)
        result = self.tracker.update(moved)          # 새 앵커 기준으론 아직 0.2초
        self.assertEqual(len(result.events), 0)

    def test_disabled_by_default_never_fires(self):
        super().setUp(dwell_enabled=False)   # recenter_dwell_enabled 기본값(False)
        self._settle_calibration()
        for _ in range(20):
            result = self.tracker.update(make_face())
            self.clock.tick(FRAME_DT_SEC)
        self.assertEqual(result.events, [])


class ResetRecenterDwellTest(HeadTrackerTestBase):
    """reset_recenter_dwell() — 재정렬 시계만 되돌리고 나머지는 안 건드린다.

    회귀 방지(2026-08-20): head.py/eyebrow.py가 클릭·드래그 때 reset_event_gates()
    (모든 판정 초기화)를 부르던 시절, 가만히 있으면 응시 클릭이 계속 나가고 그
    클릭이 재정렬 시계를 매번 0으로 되돌려서 재정렬이 영영 발화하지 못했다.
    실사용 로그 기준 27초 동안 응시 클릭 13번 / 재정렬 0번.
    """

    def setUp(self):
        # 응시 클릭(0.3초)과 재정렬(0.9초)을 함께 켜 실제 구성을 재현한다
        super().setUp(dwell_enabled=True, recenter_dwell_enabled=True,
                      recenter_dwell_sec=0.9)

    def test_resets_recenter_timer_only(self):
        self._settle_calibration()
        self.tracker.update(make_face())     # 응시 시작(앵커 설정)
        self.clock.tick(0.6)                 # 재정렬 0.9초 중 0.6초 경과
        self.tracker.reset_recenter_dwell()  # 여기서 시계가 0으로

        # reset()은 앵커를 지우므로 시계는 '되돌린 시점'이 아니라 '다음 프레임'
        # 부터 다시 흐른다 — 그 다음 프레임에서 앵커가 새로 잡힌다
        self.tracker.update(make_face())
        self.clock.tick(0.6)                 # 새 앵커 기준 0.6초 — 아직 0.9초 미만
        result = self.tracker.update(make_face())
        self.assertNotIn("calibration", [e.class_name for e in result.events],
                         "되돌렸는데도 이전 진행이 남아 있다")

        self.clock.tick(0.4)                 # 새 앵커 기준 1.0초 — 이제 넘어선다
        result = self.tracker.update(make_face())
        self.assertIn("calibration", [e.class_name for e in result.events],
                      "되돌린 뒤 다시 채웠는데도 재정렬이 안 났다")

    def test_does_not_rearm_dwell_click(self):
        """응시 클릭은 '반경을 벗어나야 다시 누름' 규칙(require_release_to_rearm)이
        걸려 있다. 재정렬 시계만 되돌리는 이 메서드는 그 규칙을 풀면 안 된다 —
        풀리면 가만히 있는 것만으로 클릭이 무한 반복된다."""
        self._settle_calibration()
        self.tracker.update(make_face())
        self.clock.tick(0.31)
        result = self.tracker.update(make_face())
        self.assertIn("select", [e.class_name for e in result.events])   # 1회 발화

        for _ in range(10):                  # 제자리에 머문 채 계속 되돌려도
            self.tracker.reset_recenter_dwell()
            self.clock.tick(0.31)
            result = self.tracker.update(make_face())
            self.assertNotIn("select", [e.class_name for e in result.events],
                             "제자리인데 응시 클릭이 재발화했다 — 재장전 규칙이 풀렸다")


class DwellClickTest(HeadTrackerTestBase):
    def test_no_event_before_dwell_duration(self):
        self._settle_calibration()
        self.tracker.update(make_face())   # 응시 시작(앵커 설정)
        self.clock.tick(0.29)
        result = self.tracker.update(make_face())
        self.assertEqual(len(result.events), 0)

    def test_fires_once_after_dwell_duration(self):
        self._settle_calibration()
        self.tracker.update(make_face())
        self.clock.tick(0.31)
        result = self.tracker.update(make_face())
        self.assertEqual(len(result.events), 1)
        self.assertEqual(result.events[0].data["trigger"], "dwell")

    def test_does_not_refire_without_leaving_radius(self):
        self._settle_calibration()
        self.tracker.update(make_face())
        self.clock.tick(0.31)
        self.tracker.update(make_face())          # 1차 확정
        self.clock.tick(0.31)
        result = self.tracker.update(make_face())  # 같은 자리 유지 — 재발화 없음
        self.assertEqual(len(result.events), 0)

    def test_refires_after_leaving_and_returning(self):
        self._settle_calibration()
        self.tracker.update(make_face())
        self.clock.tick(0.31)
        self.tracker.update(make_face())   # 1차 확정

        moved = make_face(nose_local_px=(20.0, 0.0))   # 고개를 크게 돌림 — 반경 이탈로 새 앵커
        result = self.tracker.update(moved)
        self.assertEqual(len(result.events), 0)

        self.clock.tick(0.31)
        result = self.tracker.update(moved)   # 새 위치에서 dwell_sec 재충족
        self.assertEqual(len(result.events), 1)


class CrossDetectorCooldownTest(HeadTrackerTestBase):
    def test_mouth_and_dwell_in_same_tick_fire_once(self):
        self._settle_calibration()
        self.tracker.update(make_face())
        self.clock.tick(0.31)   # dwell_sec 충족 시점에 입도 벌림
        result = self.tracker.update(make_face(jaw_open=0.8))
        select_events = [e for e in result.events if e.class_name == "select"]
        self.assertEqual(len(select_events), 1)


class AdaptiveBaselineTest(HeadTrackerTestBase):
    """고정 임계값 대신 잠금 직후 캡처한 평상시 기준선 + 여유값으로 판정한다.
    사람마다 평상시 블렌드셰이프 값이 다른 상황을 재현해 임계값이 실제로
    기준선을 따라 이동하는지 검증한다."""

    def setUp(self):
        super().setUp(dwell_enabled=False)

    def test_mouth_threshold_shifts_with_elevated_baseline(self):
        # 평상시 입이 약간 벌어져 있는 사용자(기준선 0.3) 모사
        self._settle_calibration(jaw_open=0.3)
        # 기준선(0.3) 그대로면 open_margin(0.5)을 못 넘는다
        result = self.tracker.update(make_face(jaw_open=0.3))
        self.assertEqual(len(result.events), 0)
        # 기준선 + margin을 확실히 넘겨야 확정
        result = self.tracker.update(make_face(jaw_open=0.9))
        self.assertEqual(len(result.events), 1)

    def test_eye_close_baseline_prevents_resting_false_trigger(self):
        # 평상시 eyeBlink가 높게 잡히는 사용자(기준선 0.5) 모사
        self._settle_calibration(eye_close=0.5)
        result = None
        for _ in range(20):   # 기준선 그대로 오래 유지 — 확정되면 안 된다
            result = self.tracker.update(make_face(eye_close=0.5))
            self.clock.tick(FRAME_DT_SEC)
        self.assertEqual(len(result.events), 0)
        # 기준선보다 확실히 더(거의 최대) 감아야 확정
        self.tracker.update(make_face(eye_close=1.0))
        self.clock.tick(0.31)
        result = self.tracker.update(make_face(eye_close=1.0))
        self.assertEqual(len(result.events), 1)

    def test_no_detection_fires_before_baseline_ready(self):
        # 캘리브레이션 구간 중에는(코 캘리브레이션도 아직) 입/눈 판정 자체가 보류된다
        result = self.tracker.update(make_face(jaw_open=0.9, eye_close=0.9))
        self.assertIsNone(result.cursor_x_ratio)
        self.assertEqual(result.events, [])


if __name__ == "__main__":
    unittest.main()
