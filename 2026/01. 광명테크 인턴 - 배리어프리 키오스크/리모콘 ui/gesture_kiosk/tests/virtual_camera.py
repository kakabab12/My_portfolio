...
def varied_face(interocular=1.0, width=1.0, height=1.0, nose=1.0, scale=1.0):
    ...
    해로운지 측정하려면 정규 모형과 **다른** 얼굴을 만들 수 있어야 한다.
...
# 이름의 숫자(65도·90도...)는 **부르는 별명**이지 아래 focal_px에서 계산되는
# 화각과 같은 값이 아니다. f=700은 세로 54도(원본 1280폭 기준 가로 85도),
# f=290은 세로 102도(가로 131도)다. 중요한 것은 k1의 크기와 f의 순서다
LENS_PROFILES = {
...
class VirtualCamera:
    ...
    def reset_noise(self):
        ...
        한 카메라로 여러 항목을 이어서 측정하면 난수가 진행돼, 나중에 측정하는 항목이
