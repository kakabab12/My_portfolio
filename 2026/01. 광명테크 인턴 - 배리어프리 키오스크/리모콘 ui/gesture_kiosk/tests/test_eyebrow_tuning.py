    ...
    def test_valid_file_returns_dict(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            with open(path, "w", encoding="utf-8") as f:
                json.dump({"orientation_half_span_x_deg": 18.0}, f)
            self.assertEqual(_load_tuning_overrides(path), {"orientation_half_span_x_deg": 18.0})
...
class TuningReloaderTest(unittest.TestCase):
    ...
    def test_reloads_when_file_changes(self):
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            with open(path, "w", encoding="utf-8") as f:
                json.dump({"orientation_half_span_x_deg": 12.0}, f)
            clock.advance(0.2)   # 확인 주기를 지나야 본다
            reloader.maybe_reload(tracker)
            self.assertEqual(len(tracker.calls), 1)
            self.assertEqual(tracker.calls[0]["half_span_x_deg"], 12.0)
            self.assertIsNone(tracker.calls[0]["half_span_y_deg"])   # 파일에 없던 키
