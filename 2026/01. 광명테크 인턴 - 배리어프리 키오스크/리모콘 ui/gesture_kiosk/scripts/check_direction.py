...
def main():
    ...
    def record(label, seconds):
        ...
        ★2026-09-10 — 예전에는 오른쪽 한 번만 측정하고 **사용자가 정말 오른쪽으로
        돌렸다고 가정**했다(frame_is_mirrored = d_face > 0). 그런데 테스트에서
        왼쪽으로 돌린 순간이 최댓값으로 잡히자 거울 판정이 통째로 뒤집혀
        "커서가 반대로 간다"는 정반대 결론이 나왔다. 세 번 측정하고 나서야 알았다.

        그래서 **두 방향을 다 측정하고 서로 반대인지 확인한다.** 반대가 아니면
