"""eyebrow.exe 런처 원본 — PyInstaller로 이 스크립트만 빌드해 얇은 실행기를
만든다 (2026-08-13 신설, scripts/head_launcher.py와 동일 패턴 — 그 파일
독스트링 참고).

이 exe 자체는 mediapipe·opencv 등 무거운 의존을 담지 않는다(가벼운 런처만
빌드 — venv 없이 시스템 파이썬 공유 방향 유지). 시스템 파이썬을 찾아 실제
헤드트래커 엔진(eyebrow.py, 프로젝트 루트)을 자식으로 그대로 실행하고
표준입출력을 그대로 물려준다.

빌드 방법 (make_eyebrow_exe.bat 참고 — 임시 venv에서 pyinstaller 설치 후 빌드,
시스템 파이썬은 건드리지 않는다):
    make_eyebrow_exe.bat
"""
import ctypes
import os
import subprocess
...
def _get_root_dir():
    """eyebrow.exe 자신이 있는 폴더 — 프로젝트 루트(eyebrow.py와 같은 위치를 전제)."""
...
def main():
    ...
    entry_py_path = os.path.join(root_dir, "eyebrow.py")
