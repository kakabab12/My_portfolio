"""run.exe 런처 원본 — PyInstaller로 이 스크립트만 빌드해 얇은 실행기를 만든다 (2026-08-03 신설).
...
직접 실행하는 것도 까다롭다 — 델파이 쪽에서 바로 띄울 진짜 실행 파일이
필요했다(2026-08-03 사용자 요청).

이 exe 자체는 mediapipe·numpy 등 무거운 의존을 담지 않는다(가벼운 런처만 빌드
— venv 없이 시스템 파이썬 공유 방향 유지, install.bat 참고). 시스템 파이썬을
찾아 실제 엔진(main.py — 2026-08-03부터 프로젝트 루트의 공식 진입점, 구
scripts/run_demo.py 대체)을 자식으로 그대로 실행하고 표준입출력을 그대로
물려준다 — 그래서 델파이까지 stdout이 손실 없이 전달된다(event_sender.py의
"stdout은 이벤트 전용" 규칙 유지 — 이 파일도 stdout에는 아무것도 찍지 않는다,
진단 메시지는 stderr로만).
...
def _set_console_utf8():
    ...
    — 자식 파이썬의 한글 로그(stderr)가 깨져 보인다(2026-08-03 테스트 확인). 델파이
    파서가 읽는 stdout 프로토콜은 ASCII 고정이라 무관하지만, 진단용 한글 로그
    가독성을 위해 맞춰준다.
    """
    try:
