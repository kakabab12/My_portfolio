...
근처의 한 점"(코끝 쪽으로 살짝 당긴 점)이 얼마나 벗어났는지를 측정해 커서
...
| 이름 | 뜻 |
|---|---|
| `CLICK_FREEZE_ENABLED` | 누르는 동안 커서를 붙잡을지. 끄면 클릭이 얼굴이 향한 곳에서 최대 264 px까지 벗어난다(가상 사용자 측정) |
...
| Pillow (PIL) | MIT-CMU | **가능** | 코드에서 한글 텍스트 렌더링에 직접 씀(`put_korean_text`). ⚠ **requirements.txt에 빠져 있음** — MediaPipe가 의존하는 matplotlib을 통해 딸려 들어와 있을 뿐 직접 명시되어 있지 않다. 다른 PC에 새로 설치할 때 버전이 안 맞을 위험이 있으니 requirements.txt에 추가를 권장 |
