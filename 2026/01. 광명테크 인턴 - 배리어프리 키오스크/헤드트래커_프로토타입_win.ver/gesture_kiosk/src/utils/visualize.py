...
CURSOR_COLOR = (80, 220, 120)    # 커서 점 (코/시선 — cursor_mode에 따라 출처만 다름)
...
def draw_person_lock(frame, person_lock, cursor_x_ratio, cursor_y_ratio):
    """잠긴 사용자의 얼굴 박스와 커서 위치를 그린다(코/시선 중 활성 모드 — cursor_x/y_ratio는
    출처와 무관하게 같은 형식이라 이 함수는 어느 모드인지 몰라도 된다).
...
def draw_debug_panel(frame, debug):
    ...
    if not debug:
        return frame
    h_px = frame.shape[0]
    mode_tag = "EYE" if debug.get("cursor_mode") == "eye" else "HEAD"
    cursor_tag = (
    ...
    lines = [f"CURSOR[{mode_tag}] {cursor_tag}", jaw_line, eye_line, pucker_line]
