...
- 모델: **MediaPipe FaceLandmarker(Apache-2.0) 단일** — 커서·선택·뒤로가기가 전부
  얼굴 랜드마크+블렌드셰이프 하나로 판정됩니다 (학습 0회 스택). CPU만으로 추론 ~30 FPS
  실측(2026-07-20, 처리 해상도 640x360 — KPI 30 FPS 달성. 구 RTMPose 포즈 방식은
  CPU 0.5 FPS라 GPU가 필수였습니다 — 이번 전환으로 배포가 크게 단순해졌습니다)
...
- 뒤로가기 제스처 변천: 눈썹 올리기(원안) → 입 오므리기(앞머리 가림 문제 해결) →
  ...
  입력값은 보존된다(2026-07-20) — 완료 화면에서만 예외로 처음으로 돌아갑니다.
- **2026-07-20 정확도 개선**: 입벌림·눈감김 판정이 고정 임계값 대신 "잠금 직후 캡처한
  평상시 표정 기준선 + 여유값"을 씁니다 — 사람마다 평상시 블렌드셰이프 값 편차가 커서
  (테스트 관찰: eyeBlink 0.1~0.6) 고정 임계 하나로는 오탐/미탐이 갈렸습니다.
...
- 엔진은 프레임·인식값을 저장하지 않습니다 (설치가이드.md F절)
- **라이선스 (2026-07-18 헤드트래커 전환 기준)**: 스택 전체가 상업 사용 가능 + 코드 공개
  ...
  검토 대상이 이전(rtmlib+HaGRID 시절)보다 더 단순해졌습니다.
  Apache/MIT의 라이선스 문서 동봉(배포물 내 고지)은 통상 절차 — 제품 화면 표시 의무는 없습니다.
