...
from src.inference.face_estimator import (
    ...
    LMK_RIGHT_EYE_OUTER, LMK_RIGHT_EYE_TOP, LMK_RIGHT_IRIS_CENTER, FaceLandmarks,
)
...
EYE_SPAN_PX = 20.0  # 테스트 전용 — 눈 코너/눈꺼풀 사이 고정 간격(시선 비율 계산의 분모)


def _fill_eye(landmarks_px, base_px, gaze_ratio, outer_idx, inner_idx, top_idx, bottom_idx, iris_idx):
    """한쪽 눈의 코너·눈꺼풀·홍채 랜드마크를 채운다 — _gaze_point()에 넣으면 정확히
    gaze_ratio가 그대로 나오도록 역산한 좌표다(아이트래킹 모드 테스트용, 2026-07-22)."""
    base_x, base_y = base_px
    landmarks_px[outer_idx] = (base_x, base_y)
    landmarks_px[inner_idx] = (base_x + EYE_SPAN_PX, base_y)
    landmarks_px[top_idx] = (base_x, base_y)
    landmarks_px[bottom_idx] = (base_x, base_y + EYE_SPAN_PX)
    landmarks_px[iris_idx] = (base_x + gaze_ratio[0] * EYE_SPAN_PX, base_y + gaze_ratio[1] * EYE_SPAN_PX)
...
def make_face(nose_px=(100.0, 100.0), eye_dist_px=40.0, jaw_open=0.0,
    ...
    gaze_ratio는 양쪽 눈에 동일하게 적용하는 시선 비율(x, y) — head_tracker._gaze_point()가
    양쪽 평균을 내므로 이 값 그대로가 결과가 된다(아이트래킹 모드 테스트용, 2026-07-22).
    """
    landmarks_px = np.zeros((NUM_LANDMARKS, 2), dtype=np.float32)
    landmarks_px[LMK_NOSE_TIP] = nose_px
    _fill_eye(landmarks_px, (nose_px[0] - eye_dist_px / 2.0, nose_px[1]), gaze_ratio,
              LMK_LEFT_EYE_OUTER, LMK_LEFT_EYE_INNER, LMK_LEFT_EYE_TOP, LMK_LEFT_EYE_BOTTOM,
              LMK_LEFT_IRIS_CENTER)
    _fill_eye(landmarks_px, (nose_px[0] + eye_dist_px / 2.0, nose_px[1]), gaze_ratio,
              LMK_RIGHT_EYE_OUTER, LMK_RIGHT_EYE_INNER, LMK_RIGHT_EYE_TOP, LMK_RIGHT_EYE_BOTTOM,
              LMK_RIGHT_IRIS_CENTER)
    blendshapes = {
        "jawOpen": jaw_open,
...
def make_config(smoothing_alpha=1.0, distance_smoothing_alpha=1.0, dwell_enabled=True,
                 recenter_enabled=True, cursor_mode="head",
                 gaze_sensitivity_x=2.0, gaze_sensitivity_y=2.0,
                 gaze_smoothing_alpha_x=1.0, gaze_smoothing_alpha_y=1.0):
    return {
        "head_tracker": {
            ...
            "gaze_pointer": {
                "sensitivity_x": gaze_sensitivity_x,
                "sensitivity_y": gaze_sensitivity_y,
                "smoothing_alpha_x": gaze_smoothing_alpha_x,
                "smoothing_alpha_y": gaze_smoothing_alpha_y,
                "max_offset_ratio": 0.5,
            },
...
class GazeCursorMappingTest(HeadTrackerTestBase):
    """아이트래킹(eye) 모드 커서 매핑 — head 모드(CursorMappingTest)와 같은 계약을
    시선 비율 입력으로 검증한다(2026-07-22 신설)."""

    def setUp(self):
        super().setUp(cursor_mode="eye", dwell_enabled=False)

    def _settle_gaze_calibration(self, gaze_ratio=(0.5, 0.5), generation=1, max_frames=20):
        for _ in range(max_frames):
            result = self.tracker.update(make_face(gaze_ratio=gaze_ratio), generation)
            if result.cursor_x_ratio is not None:
                return result
            self.clock.tick(FRAME_DT_SEC)
        raise AssertionError("캘리브레이션이 max_frames 안에 끝나지 않았다")

    def test_eye_mode_calibration_centers_cursor(self):
        result = self._settle_gaze_calibration()
        self.assertAlmostEqual(result.cursor_x_ratio, 0.5, places=2)
        self.assertAlmostEqual(result.cursor_y_ratio, 0.5, places=2)
        self.assertTrue(result.is_tracking)

    def test_cursor_moves_with_gaze_offset(self):
        self._settle_gaze_calibration(gaze_ratio=(0.5, 0.5))
        # 시선 비율 델타 0.25 * sensitivity(2.0) = 0.5 -> max_offset_ratio(0.5)로 클램프
        result = self.tracker.update(make_face(gaze_ratio=(0.75, 0.5)), 1)
        self.assertAlmostEqual(result.cursor_x_ratio, 1.0, places=2)
        self.assertAlmostEqual(result.cursor_y_ratio, 0.5, places=2)

    def test_head_pose_does_not_affect_gaze_cursor_while_gaze_fixed(self):
        # eye 모드의 핵심 계약: 시선 비율이 그대로면 고개(코 위치)가 움직여도 커서는
        # 그대로여야 한다 — head 모드와 구분되는 지점
        self._settle_gaze_calibration(gaze_ratio=(0.5, 0.5))
        result = self.tracker.update(make_face(nose_px=(300.0, 300.0), gaze_ratio=(0.5, 0.5)), 1)
        self.assertAlmostEqual(result.cursor_x_ratio, 0.5, places=2)
        self.assertAlmostEqual(result.cursor_y_ratio, 0.5, places=2)

    def test_recenter_resets_gaze_cursor_in_eye_mode(self):
        # recenter(입 오므리기)는 활성 매퍼를 리셋해야 한다 — eye 모드에서는 시선
        # 매퍼가 리셋되는지 검증(HeadTracker._active_mapper 계약, 2026-07-22)
        self._settle_gaze_calibration()
        result = self.tracker.update(make_face(mouth_pucker=0.9), 1)
        self.assertEqual(len(result.events), 1)
        self.assertEqual(result.events[0].class_name, "recenter")
        result = self.tracker.update(make_face(mouth_pucker=0.1), 1)
...
class CursorModeSwitchTest(HeadTrackerTestBase):
    """POST /set_cursor_mode -> HeadTracker.set_cursor_mode() 전환 계약 검증 (2026-07-22 신설)."""

    def test_default_mode_is_head(self):
        self.assertEqual(self.tracker.cursor_mode, "head")

    def test_mode_switch_applies_on_next_update_not_immediately(self):
        self._settle_calibration()
        self.tracker.set_cursor_mode("eye")
        self.assertEqual(self.tracker.cursor_mode, "head")   # 다음 update() 전까지는 아직 head
        self.tracker.update(make_face(), 1)
        self.assertEqual(self.tracker.cursor_mode, "eye")

    def test_switching_mode_resets_cursor_until_recalibrated(self):
        self._settle_calibration(nose_px=(100.0, 100.0))
        self.tracker.set_cursor_mode("eye")
        result = self.tracker.update(make_face(nose_px=(100.0, 100.0)), 1)
        # 모드가 막 바뀐 프레임 — 새 매퍼가 리셋돼 캘리브레이션부터 다시 시작한다
        self.assertIsNone(result.cursor_x_ratio)

    def test_switching_back_to_head_does_not_reuse_stale_calibration(self):
        self._settle_calibration(nose_px=(100.0, 100.0))
        self.tracker.set_cursor_mode("eye")
        self.tracker.update(make_face(nose_px=(100.0, 100.0)), 1)   # eye로 전환 반영
        self.tracker.set_cursor_mode("head")
        result = self.tracker.update(make_face(nose_px=(300.0, 300.0)), 1)
        # head로 되돌아왔지만 이전 캘리브레이션(100,100 중심)은 리셋됐으니 미확정이어야 한다
        self.assertIsNone(result.cursor_x_ratio)

    def test_invalid_mode_raises(self):
        with self.assertRaises(ValueError):
            self.tracker.set_cursor_mode("nose")
