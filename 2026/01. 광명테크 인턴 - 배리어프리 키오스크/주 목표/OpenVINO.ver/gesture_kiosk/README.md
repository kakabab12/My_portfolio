...
- 모델: **RTMPose wholebody 포즈(Apache-2.0) 단일** — 손 모양·궤적·사용자 잠금이
  전부 키포인트 하나로 판정됩니다 (손 모양도 손 21점 기하 규칙 — 별도 CNN 없음)
...
- **핵심 규칙**: 손 모양이 계층을(탐색/명령), 이동 방향이 기능을 정합니다 —
  반복 횟수·화면 좌표는 쓰지 않습니다. 탐색(한 손가락)은 아무리 반복해도 화면이
  안 바뀌고, 화면을 바꾸는 동작은 주먹을 쥐어야만 실행됩니다 (오발 안전 구조)
- 손 모양 판별은 프레임별로 흔들릴 수 있어 **궤적 창 안 다수결**로 확정합니다 —
  블러로 판별이 끊겨도(기권) 이벤트가 유실되지 않습니다
...
- 엔진은 프레임·인식값을 저장하지 않고 로그는 마스킹합니다 (설치가이드.md F절)
