...
★2026-08-05 로터 판(feat/rotor_remote — 사용자 결정): 이 판의 일반 모드 쓸기는
**손 펼침 좌/우/상**뿐이다. up은 로터(리모컨) 모드 토글로 소비되고(rotor.py),
주먹·한 손가락 계층과 쥐기 select·탭 click은 비활성(데드코드 — EVENT_BY_SHAPE와
filter_signals의 해당 호출 주석 참고). select 등 명령은 로터 4방향 버튼이 담당한다.
아래 독스트링은 비활성 이전(2026-08-03) 체계의 기록으로 남긴다.

동작 체계(2026-08-03 개편 — 사용자 결정: 탐색/temp 계층 맞바꿈 + select를 쓸기에서
모양 전환으로 이동):
- **손 펼침** + 좌/우 쓸기 = left / right — 포커스 이동(탐색 계층)
- **주먹** + 왼쪽 = back(이전) · 주먹 + 위 = home(처음으로) · 주먹 + 오른쪽 = confirm(확인)
- **한 손가락** + 좌/우/위 = temp_left / temp_right / temp_top (temp 계층)
- **손 펼침 → 주먹 2회 반복 = select** — 쓸기가 아니라 모양 전환으로 센다
  (_update_grab_select). 종전 "한 손가락 + 위 쓸기 = select"는 팔을 들어올리는
  동작과 구분이 어려워 오발이 잦았다 — 이동 궤적과 완전히 분리했다
- **한 손가락** 제자리 더블 탭 = click (_update_tap_click — 계층과 무관하게 유지)
- 아래 방향 = 정의 없음(전 모양 공통 — 07-29 bottom 소멸) · 손 펼침의 위도 정의
  없음(select가 옮겨갔다) — 무시 (복귀 삼킴만 무장해 반동 오발을 막는다)
...
체계로) → 07-29 재개편으로 위 쓸기가 select 명칭을 되찾았다가 → 08-03 다시 쓸기에서
떨어져 나와 모양 전환(펼침→주먹 2회)이 됐다. 쿨다운·반대 방향 복귀 삼킴·
들어올리기 게이트(위 방향 = home 오발 방지)·소실 유예는 유지.
수치는 config (기획서 4.7).
"""
import math
import time
...
# 손 모양 × 이동 방향 -> 이벤트.
# ★2026-08-05 로터 판(feat/rotor_remote — 사용자 결정): 일반 모드 쓸기는 **손
# 펼침 좌/우/상 3칸만** 남긴다. up은 파이프로 나가지 않고 realtime_loop의 로터
# 토글(설정 on/off)로 소비된다(rotor.py). 주먹·한 손가락 계층과 펼침 아래는
# 이 판에서 비활성 — 코드는 지우지 않고 표에서만 뺀다(아래 원본 주석을 복원하면
# 되살아난다). 빈 dict를 남기는 이유: _judge_swipe가 EVENT_BY_SHAPE[shape]를
# 직접 조회하므로 키 자체가 없으면 KeyError — 빈 칸은 "정의 없는 조합" 경로
# (무시 + 복귀 삼킴 무장)를 그대로 탄다.
EVENT_BY_SHAPE = {
    SHAPE_OPEN: {"left": "left", "right": "right", "up": "up"},
    SHAPE_FIST: {},
    SHAPE_FINGER: {},
    # -- 2026-08-05 비활성 이전 원본(2026-08-04 12칸 — "펼친손 상하좌우
    #    up,down,left,right로 넣고, 빈곳은 일단 temp_로 다 넣어줘") --
    # SHAPE_OPEN: {"left": "left", "right": "right", "up": "up", "down": "down"},
    # SHAPE_FIST: {"left": "back", "up": "home", "right": "confirm",
    #              "down": "temp_fist_bottom"},
...
class GestureFilter:
    def __init__(self, config, clock=time.monotonic):
        ...
        # 쥐기 선택(2026-08-03 사용자 결정) — 손 펼침 → 주먹 N회 = select.
        # 키(grab_select) 없으면 기능 없음 (종전 동작 — 위 쓸기 select는 제거됐다)
        grab_cfg = config["gestures"].get("grab_select")
        self._grab_window_sec = (grab_cfg.get("window_sec", 3.0)
                                 if grab_cfg is not None else None)
        self._grab_repeat_count = (grab_cfg or {}).get("repeat_count", 2)
        self._grab_max_move = (grab_cfg or {}).get("max_move_shoulder", 0.25)
        # 쥔 뒤 이만큼 그 자리에 머물러야 select — 0이면 즉시 발화(종전)
        self._grab_dwell_sec = (grab_cfg or {}).get("dwell_sec", 0.0)
        self._grab_count = 0             # 창 안에서 완성된 쥐기 횟수
        self._grab_first_sec = None      # 첫 쥐기 시각 — 창의 기준
        self._grab_anchor_point = None   # 첫 쥐기 위치 — 제자리 반경의 기준
        self._grab_prev_latched = None
        self._grab_dwell_deadline_sec = None   # 쥔 뒤 정지 유예 마감 시각
        self._grab_dwell_point = None          # 쥔 자리 — 여기서 벗어나면 쓸기다
        self._grab_dwell_canceled_count = 0    # 계기판 — 이동/펴기로 취소된 수   # 직전 프레임의 래치 — 전환 시점 검출
        self._has_seen_open = False      # 펼침을 본 뒤인가 (주먹 도달 시 1회로 계수)
        ...
        self._undefined_ignored_count = 0  # 계기판 — 정의 없는 조합으로 무시한 수
                                           #   (12칸을 다 채운 뒤로는 0이어야 정상)
    ...
    def filter_signals(self, hand_signal, shoulder_width_ratio=None,
        ...
        if self._is_in_cooldown(now_sec):
            # 쿨다운 중엔 **판정만** 멈춘다 — 궤적은 계속 쌓는다.
            #
            # ★2026-08-04 재작성(쿨다운 인하의 전제): 종전엔 궤적도 안 쌓았다.
            # 그러면 쿨다운이 끝나는 순간 궤적이 복귀 **도중부터** 시작해,
            # "이 획이 직전 획의 끝을 지나왔는가"라는 삼킴의 근거
            # (_is_return_from_origin)가 통째로 사라진다. 쿨다운(0.9초)이 복귀
            # 동작보다 길어서 우연히 가려져 있었을 뿐이라, 쿨다운을 줄이자마자
            # 되돌아오는 팔이 반대 이벤트로 튀었다(시나리오 12가 잡았다).
            # 증거를 남겨 두면 삼킴이 설계대로 동작하고, 오래된 점은 창
            # (window_sec)이 알아서 만료시킨다 — 정지 후의 의도적 반대 쓸기는
            # 그 사이 궤적이 비므로 정상 발화한다.
            # 확정만 막는 것이므로 탭·쥐기 등 다른 판정도 종전대로 쉰다.
            if hand_signal is not None:
                point = hand_signal[1]
                if self._point_filter is not None:
                    point = self._point_filter.filter(point, now_sec)
                self._update_swallow_origin(point)
                self._stamp_rest_zone(point, now_sec, body_scale)
                self._swipe_tracker.update(point[0], point[1], now_sec, body_scale)
            return None
        ...
        if hand_signal is None:
            ...
            # ★2026-08-05 로터 판 비활성(사용자 결정): 쥐기 select·탭 click은 이
            # 판에서 쓰지 않는다 — select는 로터 좌 버튼이 담당한다(rotor.py).
            # 판정 함수·config 키는 그대로 두고 호출만 잘라 데드코드로 남긴다 —
            # 아래 주석을 복원하면 되살아난다.
            # grab_event = self._update_grab_select(point, now_sec, body_scale)
            ...
            #     self._update_debug(body_scale, shoulder_width_ratio)
            #     return tap_event
            _ = index_ratio   # 신호 형식은 유지 — 탭 복원 시 그대로 쓴다
    ...
    def _judge_swipe(self, direction, label, now_sec, point, body_scale):
        ...
        self._set_swallow(direction, now_sec, point, stroke_start)
        return self._confirm(event_name, 1.0, now_sec, hand_side=label)
    ...
    # ----- 쥐기 선택 (2026-08-03 사용자 결정 — 손 펼침 → 주먹 2회 = select) -----

    def _update_grab_select(self, point, now_sec, body_scale):
        """손을 폈다 쥐는 동작 N회 -> "select" | None. 래치가 바뀐 프레임에서만 부른다.

        ★2026-08-03 스펙 변경(사용자): 종전 select는 "한 손가락 + 위 쓸기"였는데,
        들어올리기와 같은 동작이라 오발이 잦았다. 쓸기가 아닌 **모양 전환**으로
        옮겨 이동 궤적과 완전히 분리한다 (위 쓸기 select는 제거 — EVENT_BY_SHAPE).

        판정을 **래치 전환**으로 세는 이유: 프레임별 판별은 손을 쥐는 도중
        출렁이므로 그대로 세면 한 번 쥔 것이 여러 번으로 계수된다. 래치는 이미
        연속 확인(switch_frames)을 통과한 신호라 한 번의 쥐기가 한 번으로 남는다.

        "펼침을 봤다"를 **깃발로** 들고 있다가 주먹 도달에서 1회로 세는 구조다 —
        손을 쥐는 중간 자세가 잠깐 한 손가락으로 읽혀 펼침→손가락→주먹으로
        전환돼도 한 번으로 세어진다 (펼침→주먹 직접 전환만 세면 놓친다).

        ★제자리 조건(max_move_shoulder — 탭과 같은 방어): 명령 계층을 쓰는
        자연스러운 흐름(펼침으로 탐색 → 주먹으로 confirm → 다시 펼침 → 다시 주먹)이
        그대로 "쥐기 2회"라 창 안에 들어오면 select가 오발된다(테스트로 재현했다).
        명령은 반드시 **쓸기를 동반**하므로 이동으로 갈린다 — 단 쥐는 **순간의**
        위치만 보면 안 된다: 쓸고 제자리로 돌아와 쥐면 두 쥐기의 좌표가 같다.
        그래서 이 함수는 매 프레임 불려 **쥐기 사이의 이동 전체**를 감시한다.

        ★2026-08-04 정지 유예 신설(사용자 결정 "select 2번에서 1번으로"). 1회로
        내리면 위 제자리 조건이 통째로 무력해진다 — 그 조건은 쥐기와 쥐기 **사이**만
        보는데 1회엔 사이가 없기 때문이다. 그러면 주먹을 만드는 순간 select가 나가
        back·home·confirm이 매번 오발을 달고 다닌다(명령은 반드시 주먹을 거친다).
        그래서 제자리 조건을 "쥐기 사이"에서 **"쥔 뒤"** 로 옮겼다: 마지막 쥐기에서
        바로 발화하지 않고 grab_dwell_sec 동안 그 자리에 머무는지 본다. 쥐자마자
        쓸면 이동으로 갈려 명령만 나가고, 쥐고 멈추면 select가 나간다.
        """
        if self._grab_window_sec is None:
            return None
        dwell_event = self._resolve_grab_dwell(point, now_sec, body_scale)
        if dwell_event is not None:
            return dwell_event
        latched = self._latched_shape
        is_latch_changed = latched != self._grab_prev_latched
        self._grab_prev_latched = latched
        if self._grab_count > 0:
            is_stale = now_sec - self._grab_first_sec > self._grab_window_sec
            is_moved = (self._grab_anchor_point is not None and body_scale > 0.0
                        and math.dist(point, self._grab_anchor_point)
                        > self._grab_max_move * body_scale)
            if is_stale or is_moved:
                self._grab_count = 0
                self._grab_first_sec = None
                self._grab_anchor_point = None
                # 지금 펼친 상태라면 그 사실은 살린다 — 다음 쥐기가 첫 회가 된다
                self._has_seen_open = latched == SHAPE_OPEN
        if not is_latch_changed:
            return None
        if latched == SHAPE_OPEN:
            self._has_seen_open = True
            return None
        if latched != SHAPE_FIST or not self._has_seen_open:
            return None
        self._has_seen_open = False
        if self._grab_count == 0:
            self._grab_first_sec = now_sec
            self._grab_anchor_point = point
        self._grab_count += 1
        if self._grab_count < self._grab_repeat_count:
            return None
        if self._grab_dwell_sec > 0.0:
            self._grab_dwell_deadline_sec = now_sec + self._grab_dwell_sec
            self._grab_dwell_point = point
            return None   # 그 자리에 머무는지 보고 발화 (_resolve_grab_dwell)
        self._reset_grab()
        return self._confirm("select", 1.0, now_sec, hand_side=self._hand_label)

    def _resolve_grab_dwell(self, point, now_sec, body_scale):
        """쥔 뒤 정지 유예를 확정하거나 취소한다 -> 이벤트 | None.

        취소 조건은 둘: **움직였다**(쓸기로 넘어갔다 — 명령이지 select가 아니다),
        **주먹을 풀었다**(래치가 주먹을 벗어났다). 유예를 넘기도록 주먹으로 그
        자리에 있으면 select다.
        """
        if self._grab_dwell_deadline_sec is None:
            return None
        is_moved = (self._grab_dwell_point is not None and body_scale > 0.0
                    and math.dist(point, self._grab_dwell_point)
                    > self._grab_max_move * body_scale)
        if is_moved or self._latched_shape != SHAPE_FIST:
            self._reset_grab()
            self._grab_dwell_canceled_count += 1
            return None
        if now_sec < self._grab_dwell_deadline_sec:
            return None
        self._reset_grab()
        return self._confirm("select", 1.0, now_sec, hand_side=self._hand_label)
    ...
    def _reset_grab(self):
        ...
        self._grab_anchor_point = None
        self._has_seen_open = False
        self._grab_dwell_deadline_sec = None
        self._grab_dwell_point = None
    ...
    def _clear_shape_latch(self):
        ...
        self._latch_candidate_shape = None
        self._latch_candidate_count = 0
        self._reset_grab()   # 쥐기 횟수도 손과 함께 — 다음 사용자에게 승계 금지
    ...
    def _update_debug(self, body_scale, shoulder_width_ratio):
        ...
        self.debug = {
            ...
            "grab_dwell_cancel": self._grab_dwell_canceled_count,  # 쥔 뒤 이동/펴기로 취소
            ...
            # 쥐기 선택 진단(2026-08-03 신설 — window_sec가 실기 미측정이라 필요):
            # 폈다 쥘 때마다 grabs가 올라야 한다. 안 오르면 래치 전환이 안 된 것
            # (switch_frames ↓), 오르다 0으로 돌아가면 창 초과·이동(제자리 반경)
            "grab_count": self._grab_count,
            "grab_repeat": self._grab_repeat_count if self._grab_window_sec else None,
        }
