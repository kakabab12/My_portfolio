...
★2026-08-05 로터 판(feat/rotor_remote — 사용자 결정): 일반 모드 쓸기는 손 펼침
좌/우/상뿐이다. 주먹·한 손가락 계층, 펼친 손 아래, 쥐기 select, 탭 click은
비활성(데드코드) — 해당 테스트는 SKIP_ROTOR_BRANCH로 건너뛴다(삭제하지 않는
이유: 기능을 되살릴 때 이 테스트들이 그대로 회귀 게이트가 된다).
...
이벤트: 모양(손 펼침/주먹/한 손가락) × 방향 + 탭 click + 쥐기 select.
2026-08-03 개편: 탐색↔temp 계층 맞바꿈, select는 쓸기에서 모양 전환으로 이동.
...
SKIP_ROTOR_BRANCH = ("2026-08-05 로터 판(feat/rotor_remote): 주먹·한손가락 계층, "
                     "펼친손 아래, 쥐기 select, 탭 click 비활성 — 사용자 결정 "
...
class GestureFilterTestBase(unittest.TestCase):
    ...
    def _feed_swipe(self, points, shape="open", shapes=None, label="right",
                    dt_sec=FRAME_DT_SEC, shoulder_width_ratio=None, index_ratios=None):
        """추적 손 궤적 점들을 순서대로 공급 — 첫 확정 이벤트를 돌려준다.

        shape: 전체 프레임 공통 손 모양 ("open"/"fist"/"finger"/None=불명).
               기본값은 **손 펼침** — 2026-08-03 개편으로 탐색 계층(left/right)이
               손 펼침으로 옮겨왔다. 쓸기 역학 테스트들이 이 기본값을 쓴다.
...
class OpenPalmSwipeTest(GestureFilterTestBase):
    """손 펼침(탐색 계층 — 2026-08-03 개편) — 좌/우 = left/right, 위·아래 = 정의 없음."""
    ...
    def test_open_up_fires_up(self):
        # ★2026-08-04 사용자 결정: 손 펼침이 4방향 포커스 이동을 다 맡는다.
        # 08-03에 사라졌던 위 쓸기가 select가 아니라 **up**으로 돌아왔다 —
        # 들어올리기 오발은 종전대로 휴식 존 게이트가 막는다(시나리오 16·17)
        event = self._feed_swipe(path(0.8, 0.3, 8, x_ratio=0.5))
        self.assertEqual(event.class_name, "up")

    @unittest.skip(SKIP_ROTOR_BRANCH)
    def test_open_down_fires_down(self):
        # ★2026-08-04 아래 방향 신설(07-29 bottom 제거를 되돌림). 팔을 내리는
        # 동작과의 혼동은 골반선 가드가 막는다 (_is_arm_lowering — 시나리오 22)
...
class FistCommandTest(GestureFilterTestBase):
    """주먹(명령 계층) — 왼쪽=back · 위=home · 오른쪽=confirm(구 ok) · 아래=temp."""
    ...
    def test_fist_down_fires_temp(self):
        # ★2026-08-04: 회사 정의가 없는 칸은 temp_ 로 자리만 잡는다 (사용자 결정 —
        # "빈곳은 일단 temp_ 로 다 넣어줘"). 이름은 EVENT_BY_SHAPE 한 곳에서 바꾼다
        event = self._feed_swipe(path(0.3, 0.8, 8, x_ratio=0.5), shape="fist")
        self.assertEqual(event.class_name, "temp_fist_bottom")
...
@unittest.skip(SKIP_ROTOR_BRANCH)
class FingerTempTest(GestureFilterTestBase):
    """한 손가락(temp 계층 — 2026-08-03 개편으로 손바닥에서 옮겨옴) — temp_left/right/top."""

    def test_finger_left_fires_temp_left(self):
        event = self._feed_swipe(path(0.6, 0.2, 8, y_ratio=0.4), shape="finger")
        self.assertIsNotNone(event)
        self.assertEqual(event.class_name, "temp_left")

    def test_finger_right_fires_temp_right(self):
        event = self._feed_swipe(path(0.2, 0.6, 8, y_ratio=0.4), shape="finger")
        self.assertIsNotNone(event)
        self.assertEqual(event.class_name, "temp_right")

    def test_finger_up_fires_temp_top(self):
        event = self._feed_swipe(path(0.8, 0.3, 8, x_ratio=0.5), shape="finger")
        self.assertIsNotNone(event)
        self.assertEqual(event.class_name, "temp_top")

    def test_finger_down_fires_temp(self):
        # ★2026-08-04: 한 손가락 계층은 네 방향이 모두 temp_ 로 채워졌다
        event = self._feed_swipe(path(0.3, 0.8, 8, x_ratio=0.5), shape="finger")
        self.assertEqual(event.class_name, "temp_bottom")


@unittest.skip(SKIP_ROTOR_BRANCH)
class GrabSelectTest(GestureFilterTestBase):
    """쥐기 선택 — 손 펼침 → 주먹 2회 = select (2026-08-03 사용자 결정).

    종전 select는 "한 손가락 + 위 쓸기"였는데 팔을 들어올리는 동작과 구분이
    어려워 오발이 잦았다. 쓸기가 아니라 **래치 전환**을 세어 이동 궤적과
    완전히 분리한다 — 그래서 이 테스트는 손을 **정지**시킨 채 모양만 바꾼다.
    """
    ...
    def setUp(self):
        super().setUp()
        config = make_config({"latch_frames": self.LATCH_FRAMES,
                              "switch_frames": self.SWITCH_FRAMES})
        config["gestures"]["grab_select"] = {"repeat_count": 2, "window_sec": 3.0,
                                            "max_move_shoulder": 0.25}
        self.filter = GestureFilter(config, clock=self.clock)

    def _grab(self, count=2, hold_frames=None, shape_between=None, gap_sec=0.0):
        """정지 상태에서 (펼침 → 주먹)을 count회 반복 — 첫 확정 이벤트를 돌려준다.

        shape_between: 펼침과 주먹 사이에 끼는 중간 모양 (손을 쥐는 도중의 오판 재현).
        gap_sec: 각 쥐기 사이에 흘려보낼 시간 — 시간 창 검증용.
        """
        hold = hold_frames if hold_frames is not None else self.SWITCH_FRAMES + 1
        for grab_idx in range(count):
            shapes = ["open"] * hold
            if shape_between is not None:
                shapes += [shape_between] * hold
            shapes += ["fist"] * hold
            event = self._feed_swipe([self.POINT] * len(shapes), shapes=shapes)
            if event is not None:
                return event
            if gap_sec > 0.0 and grab_idx < count - 1:
                self.clock.tick(gap_sec)
        return None

    def test_two_grabs_fire_select(self):
        event = self._grab()
        self.assertIsNotNone(event)
        self.assertEqual(event.class_name, "select")
        self.assertEqual(event.hand_side, "right")

    def test_single_grab_does_not_fire(self):
        self.assertIsNone(self._grab(count=1))

    def test_grab_through_finger_still_counts(self):
        # 손을 쥐는 도중 한 손가락으로 잠깐 읽혀도 1회로 센다 — "펼침을 봤다"를
        # 깃발로 들고 주먹 도달에서 세기 때문 (직접 전환만 세면 놓친다)
        event = self._grab(shape_between="finger")
        self.assertIsNotNone(event)
        self.assertEqual(event.class_name, "select")

    def test_grabs_outside_window_do_not_accumulate(self):
        # 창(3초)을 넘겨 띄엄띄엄 쥐면 select가 아니다 — 무심코 쥐었다 폈다 하는
        # 동작이 쌓여 발화하면 안 된다
        self.assertIsNone(self._grab(gap_sec=4.0))

    def test_holding_fist_does_not_repeat(self):
        # 주먹을 계속 쥐고 있어도 1회다 — 래치가 안 바뀌면 세지 않는다
        shapes = ["open"] * 4 + ["fist"] * 40
        self.assertIsNone(self._feed_swipe([self.POINT] * len(shapes), shapes=shapes))

    def test_fist_without_open_does_not_count(self):
        # 펼침을 본 적 없이 주먹으로 시작하면 세지 않는다 (명령 계층 진입일 뿐)
        shapes = ["finger"] * 4 + ["fist"] * 4 + ["finger"] * 4 + ["fist"] * 4
        self.assertIsNone(self._feed_swipe([self.POINT] * len(shapes), shapes=shapes))

    def test_command_layer_usage_does_not_fire_select(self):
        # ★오발 방어: 명령 계층을 쓰는 자연스러운 흐름 — 펼침으로 탐색(쓸기) →
        # 주먹 쥐고 confirm(쓸기) → 다시 펼침 → 다시 주먹. 모양만 보면 "쥐기 2회"라
        # 창(3초) 안에 들면 select가 튄다. 제자리 반경이 이걸 갈라낸다
        hold = self.SWITCH_FRAMES + 1
        events = []
        for _ in range(2):
            events.append(self._feed_swipe([(0.2, 0.4)] * hold, shape="open"))
            events.append(self._feed_swipe(path(0.2, 0.6, 8, y_ratio=0.4), shape="open"))
            self.clock.tick(1.1)   # 쿨다운
            events.append(self._feed_swipe([(0.6, 0.4)] * hold, shape="fist"))
            events.append(self._feed_swipe(path(0.6, 0.2, 8, y_ratio=0.4), shape="fist"))
            self.clock.tick(1.1)
        fired = [e.class_name for e in events if e is not None]
        self.assertNotIn("select", fired)   # 쓸기 이벤트는 나가도 select는 안 된다

    def test_disabled_without_config_key(self):
        # 키(grab_select) 없으면 기능 없음 — 종전 동작으로 되돌릴 수 있어야 한다
        config = make_config({"latch_frames": self.LATCH_FRAMES,
                              "switch_frames": self.SWITCH_FRAMES})
        self.filter = GestureFilter(config, clock=self.clock)
        self.assertIsNone(self._grab())
...
class HandShapeLatchTest(GestureFilterTestBase):
    ...
    def test_blur_gaps_do_not_lose_event(self):
        ...
        shapes = [None, "open", None, None, "open", None, None, None, "open"]
    ...
    def test_shape_change_does_not_reset_track(self):
        # 주먹↔손 펼침 전환은 손 중심 좌표가 연속 — 궤적을 리셋하지 않는다.
        ...
        shapes = ["open"] * 2 + ["fist"] * 7
        event = self._feed_swipe(path(0.2, 0.48, 8, y_ratio=0.4), shapes=shapes)
        self.assertIsNotNone(event)
        self.assertEqual(event.class_name, "confirm")

    def test_noisy_fist_frames_do_not_hijack_navigation(self):
        # 래치 핵심: 항법(손 펼침 고정) 중 주먹 오판별 4프레임 연속이 끼어도 —
    ...
    def test_sustained_opposite_shape_switches_latch(self):
        ...
        # 손을 펴고 탐색하다 주먹을 분명히 쥐면 명령 계층(confirm)으로 넘어간다
        self._use_latch(latch_frames=2, switch_frames=4)
        self._feed_swipe([(0.3, 0.4)] * 4, shape="open")   # 손 펼침 고정
    ...
    def test_freeze_ignores_shapes_during_fast_move(self):
        ...
        # 정지 때 고정한 손 펼침이 유지 — right가 나간다 (동결 없으면 confirm 오발)
        self._use_latch(latch_frames=1, switch_frames=1, freeze_speed=1.0)
        self._feed_swipe([(0.3, 0.4)] * 3, shape="open")   # 정지 — 손 펼침 고정
    ...
    def test_pointing_at_screen_keeps_navigation(self):
        # 손을 펴 보였다가(고정) 화면을 가리키며 쓸면(판별 전부 기권 — 관측
        # 없음) 래치가 그대로 유지돼 항법이 끊기지 않는다 (구 모양 기억의 계승 —
        # 래치는 시간 만료가 없어 더 안정적)
        self._feed_swipe([(0.3, 0.4)] * 6, shape="open")   # 정지 — 모양 고정
        event = self._feed_swipe(path(0.3, 0.7, 8, y_ratio=0.4), shape=None)
        self.assertIsNotNone(event)
        self.assertEqual(event.class_name, "right")
    ...
    def test_latch_cleared_when_hand_disappears(self):
        ...
        self._feed_swipe([(0.3, 0.4)] * 6, shape="open")
    ...
    def test_latch_cleared_after_release_expires(self):
        ...
        self._feed_swipe([(0.3, 0.4)] * 4, shape="open")
...
class FirstLineTest(GestureFilterTestBase):
    ...
    def test_still_rearm_moves_origin(self):
        # 우로 나가다 멈추면 그 자리가 새 원점 — 이어지는 위 쓸기가 확정된다.
        # ★2026-08-05 로터 판: 주먹 계층 비활성으로 손 펼침+위(up)로 잰다 —
        # 검증 대상은 이벤트 종류가 아니라 **원점 재장전**이다 (2026-08-03의
        # 주먹(home) 측정도 같은 이유의 이관이었다)
        self._use_first_line()
        points = [(0.2, 0.4)] * 4
