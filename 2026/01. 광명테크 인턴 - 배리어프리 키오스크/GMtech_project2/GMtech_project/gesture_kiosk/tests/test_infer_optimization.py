...
class RoiZoomTest(unittest.TestCase):
    ...
    def test_no_anchor_uses_full_frame(self):
        ...
        self.assertIsNone(resolve_roi_box(None, None, 1280, 720, self.CFG, 200.0))
    ...
    def test_far_anchor_crops_around_reach(self):
        ...
        box = resolve_roi_box(None, (620, 180, 660, 220), 1280, 720, self.CFG, 200.0)
    ...
    def test_near_anchor_bypasses_to_full_frame(self):
        ...
        self.assertIsNone(
            resolve_roi_box(None, (580, 140, 700, 260), 1280, 720, self.CFG, 600.0))

    def test_tiny_head_respects_min_side(self):
        # 초원거리(머리 20px) — 반변이 min_side/2(160)로 바닥: 배경만 확대 방지
        box = resolve_roi_box(None, (630, 190, 650, 210), 1280, 720, self.CFG, 100.0)
    ...
    def test_hysteresis_keeps_window_on_small_drift(self):
        ...
        prev = resolve_roi_box(None, (620, 180, 660, 220), 1280, 720, self.CFG, 200.0)
        drifted = resolve_roi_box(prev, (630, 185, 670, 225), 1280, 720, self.CFG, 200.0)
        self.assertEqual(drifted, prev)

    def test_window_moves_on_large_shift(self):
        # 앵커 대이동(280px) — 창 재중심 (사용자 이동 추종)
        prev = resolve_roi_box(None, (620, 180, 660, 220), 1280, 720, self.CFG, 200.0)
        moved = resolve_roi_box(prev, (900, 180, 940, 220), 1280, 720, self.CFG, 200.0)
