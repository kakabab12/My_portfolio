...
- **핵심 규칙**: 손 모양이 계층을(탐색/명령), 이동 방향이 기능을 정합니다 —
  반복 횟수·화면 좌표는 쓰지 않습니다. 탐색(손 펼침)은 아무리 반복해도 화면이
  안 바뀌고, 화면을 바꾸는 동작은 주먹을 쥐어야만 실행됩니다 (오발 안전 구조)
- **2026-08-04 개편**(사용자 결정) 두 가지: ①**12칸을 다 채웠다** — 손 펼침이
  4방향 포커스 이동을 맡고, 회사 정의가 없는 칸은 `temp_` 접두어로 자리만 잡았습니다
  (이벤트 **14종**으로 증가 — 델파이 분기 4개 추가 필요) ②**select를 쥐기 1회로**
  — 대신 쥔 뒤 그 자리에 잠깐 머물러야 합니다. 쥐자마자 쓸면 명령(back·home·confirm)
  으로 갈립니다. 이 정지 조건이 없으면 주먹을 만드는 순간 select가 나가 명령 3종이
  매번 오발을 달고 다닙니다
  ⚠ **아래 쓸기(down·temp_*_bottom)는 팔을 내리는 동작과 구분되지 않는다** —
  실측 2회로 확인했고(위치·거리 모두 겹침) 사용자 결정으로 오발을 감수합니다.
...
- 아래 방향은 전 모양 정의 없음(07-29 bottom 제거) · 손 펼침의 위도 정의 없음
  (select가 옮겨갔습니다) — 무시. 위 방향(home)은 팔 들어올리기 오발을 휴식 존
  게이트가 막습니다
- 사용자 손 선별(hand_select): **단일 손 추적**(07-31 라벨 제거) — 움직여서
  지시한 손 하나를 공간 연속성으로 고정합니다. 교체는 그 손을 내리거나,
  **손을 놓아둔 채(연속 유휴) 다른 손이 지시할 때**만 (2026-08-03 인계 규칙).
  ...
  판정 영역이 팔을 따라갑니다). 2026-08-04부터 **포즈로 2명을 보고** 손마다 주인을 가립니다
