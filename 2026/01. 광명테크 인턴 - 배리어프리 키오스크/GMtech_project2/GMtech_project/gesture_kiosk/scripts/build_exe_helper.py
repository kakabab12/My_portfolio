"""run.exe 빌드 로직 — make_exe.bat이 이 스크립트 하나만 호출한다 (2026-08-07 신설).

install.bat과 같은 이유: cmd.exe가 한글이 섞인 배치 파일을 처리하다 위치 계산이
틀어지는 버그가 있어(install.bat 헤더 주석 참고) 실질 로직은 전부 파이썬으로 옮긴다.

PyInstaller는 시스템 파이썬에 깔지 않는다(install.bat의 "가상환경 제거" 방향
유지) — 빌드 전용 임시 venv에만 설치하고 끝나면 지운다. 결과물(run.exe)만
프로젝트 루트에 남는다.
"""
import os
import shutil
...
# make_exe.bat이 chcp 65001을 거치지 않고 이 스크립트가 직접 호출되는 경로(예:
# 터미널에서 py scripts\build_exe_helper.py)에서는 콘솔 코드페이지가 시스템
# 기본(cp949)로 남아 한글 print()가 UnicodeEncodeError를 낸다(2026-08-06 실측 —
# 빌드 자체는 끝난 뒤 마지막 완료 메시지에서 발생). 표준 경로(make_exe.bat)는
# 무관하지만 방어적으로 맞춰둔다
