...
웹캠 하나로 사람 팔을 추적해 SO-ARM-101 6축 로봇팔을 실시간으로 제어하는 프로젝트입니다.
깊이 카메라 없이 일반 웹캠만 사용합니다.
...
| 손목 방향을 구분 못 함 | 부호 없는 각도 사용 | 외적 기반 부호 각도 |
...
| 버그 | 증상 | 수정 |
| --- | --- | --- |
| `remap()` 내림차순 미지원 | 입력 범위가 내림차순이면 항상 0을 반환 | `abs(span)`로 판정 |
| 감마 지수 뒤집힘 | 어두운 영상이 더 어두워짐 | 지수를 직접 적용 |
| `wrist_flex` 부호 없는 각도 | 위/아래 방향을 구분할 수 없음 | 외적 기반 부호 각도 |
