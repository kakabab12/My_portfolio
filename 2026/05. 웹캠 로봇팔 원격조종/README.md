...
**장갑도 센서도 없이, 웹캠의 2D 좌표(x, y)만으로 6축 로봇팔을 조종합니다**
...
| `wrist_roll` | 손 회전 | 고정 (2D 좌표만으로는 회전축 추정 불가) |
...
| 항목 | 내용 |
|---|---|
| **떨림 제거** | **1€ Filter**(속도 적응형) + **SmoothDamp** 2단계 필터링 |
| **실시간 튜닝** | 실행 중 키보드로 방향·0점·배율을 조정하고 파일로 저장 |
| **의존성 최소화** | 깊이 카메라·IMU 장갑 없이 **일반 웹캠 1대**만 필요 |
...
**주요 파일** — `arm_tracker.py`(관절각 계산) · `teleop_so101.py`(로봇 전송) ·
`download_models.py`(MediaPipe 모델 받기) · `tuning.json`(조정값)
