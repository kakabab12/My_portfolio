...
**인식 → 판단 → 제어 → 화면**의 전 구간을 직접 만들어 봤다.

**한 줄 소개** — 개발 PC에서만 되는 코드가 아니라, 제약이 있는 실제 하드웨어에서
끝까지 돌아가게 만든다. 그래픽카드 없는 키오스크·젯슨 보드·라즈베리파이처럼 자원이
부족한 환경에서 **성능을 직접 실측**하고, 원인을 끝까지 추적해 **현장에 배포**했다.
...
> **[실행 가이드 →](실행_가이드.md)** — 각 폴더를 어떻게 돌리는지, 저장소에 무엇이
> 빠져 있고 어떻게 되살리는지, 하드웨어 없이 어디까지 볼 수 있는지 정리했다.
