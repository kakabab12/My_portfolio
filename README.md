...
저는 **카메라로 사람의 움직임을 읽어 기계를 움직이는 일**을 주로 해 왔습니다.
그동안 만든 것들은 다음과 같습니다.
...
이 프로젝트들에는 공통점이 하나 있습니다. **전부 성능이 부족한 하드웨어에서
돌려야 했다는 점입니다.** 민원발급기에는 그래픽카드가 없고 라즈베리파이는 그봅니다
...
화면이 끊기길래 원인을 짐작해 최적화했는데 소용이 없었습니다. 직접 측정해 보니
예상과 전혀 다른 곳이 병목이었습니다.
...
개발일지 30여 편과 오류 모음을 함께 썼습니다. 잘된 것만이 아니라
**실패한 시도와 되돌린 결정도** 적었습니다.
...
**성능보다 라이선스를 택했습니다.** 쓰던 라이브러리가 AGPL이라, 그대로 두면
회사 제품 소스까지 공개해야 할 수 있었습니다. 아쉽지만 안전한 쪽으로 전부 바꿨습니다.

**감으로 고치다 멈추고 도구를 만들었습니다.** 커서가 휘는 문제를 어림값으로 두 번
손봤다가 오히려 나빠졌습니다. 세 번째엔 추측을 접고 측정하는 도구부터 만들었고,
그러다 진짜 원인을 찾았습니다.
...
만드는 입력 장치입니다. **일반 웹캠 1대만** 쓰고 별도 센서는 없습니다.
...
세 번째 추측을 하는 대신, 곡률을 실제로 측정하는 도구(`measure_arc.py`)를 만들어
