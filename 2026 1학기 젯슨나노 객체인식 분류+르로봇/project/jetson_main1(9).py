...
# [핵심 1] 1캠은 변환 성공한 엔진 사용, 2캠은 깨짐 방지를 위해 안전한 원본 .pt 파일 사용!
...
# [핵심 2] 2캠을 원본 .pt로 돌렸으니 신뢰도를 원래대로(0.60) 올려서 쓰레기를 차단합니다.
CONF_CAM1      = 0.50  
CONF_CAM2      = 0.60  
...
def send_command_to_lerobot(command, weight):
    ...
    now = time.time()
    if (now - last_command_time) < COMMAND_COOLDOWN: return
    ...
    if lerobot:
        ...
        last_command_time = now
        action_log_1.append({"action": f"{command} ({weight}g)", "result": "대기(시리얼X)"})
        if len(action_log_1) > 200: action_log_1.pop(0)
...
def run_inference(model, frame, conf_thresh, max_det=10):
    ...
    results = model.predict(frame, conf=conf_thresh, iou=0.45, max_det=max_det, verbose=False, device=DEVICE)
...
def process_frame(frame, model, cam_id=1):
    ...
    if cam_id == 1:
        ...
        if weight is not None:
            ...
            color = (0,255,0) if over else (0,180,255)
            cv2.putText(annotated, f"{weight}g (Limit:{WEIGHT_THRESHOLD}g)",
                        (20,40), cv2.FONT_HERSHEY_SIMPLEX, 1.2, color, 3)
            if over:
                send_command_to_lerobot(LEROBOT_COMMAND, weight)

    else:
        # CAM2: 상자 탐지 (.pt 원본 사용으로 안정성 확보)
        boxes1, scores1, cls1 = run_inference(model, frame, CONF_CAM2, max_det=5)

        for (cx, cy, w, h), score, cls_id in zip(boxes1, scores1, cls1):
            
            # [핵심 3] 한글 "알수없음"을 지우고 영어 "Unknown"으로 바꿔 글자 깨짐 방지!
            cls_name = BOX_LABEL_MAP.get(cls_id, f"Unknown(ID:{cls_id})")
            
            # 만약 족보(0,1)에 없는 이상한 객체가 잡히면 그냥 무시(화면에 안 그림)
...
if __name__ == "__main__":
    ...
    print(f"[INFO] FastAPI 서버 및 실시간 가동 완료 (.pt 복구됨)")
