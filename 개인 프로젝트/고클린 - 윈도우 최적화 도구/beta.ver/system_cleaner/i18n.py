...
STR: dict[str, tuple[str, str]] = {
    ...
    "busy_reject":  ("Another task is still running. Try again when it finishes.",
                     "다른 작업이 진행 중입니다. 끝난 뒤 다시 눌러주세요."),
    "working":      ("Working...", "처리하는 중..."),
    "clean_limited":("Without admin rights only your own temp files are cleaned - "
                     "DISM and disk optimization are skipped.",
                     "관리자 권한이 없으면 사용자 임시 파일만 정리하고 DISM·디스크 최적화는 건너뜁니다."),
    "svc_limited":  ("Changing services needs administrator rights. Viewing works.",
                     "서비스 목록은 볼 수 있지만, 변경하려면 관리자 권한이 필요합니다."),
    "prog_search":  ("Search programs", "프로그램 검색"),
    "prog_no_cmd":  ("This program has no registered uninstaller.",
                     "제거 명령이 등록되어 있지 않은 프로그램입니다."),
    "backup_note":  ("A backup is saved to {p} before the change.",
                     "변경 전에 {p} 에 백업을 남깁니다."),
    "twk_explorer_btn": ("RESTART EXPLORER", "탐색기 다시 시작"),
    "twk_explorer_q":   ("Restart Windows Explorer?", "탐색기를 다시 시작할까요?"),
    "twk_explorer_d":   ("Open folder windows will close and the taskbar will blink once.",
                         "열려 있는 폴더 창이 모두 닫히고 작업 표시줄이 잠깐 사라졌다 나타납니다."),
    "twk_restart_hint": ("Restart Explorer to apply.", "탐색기를 다시 시작하면 적용됩니다."),
    "disk_smart_admin": ("Detailed S.M.A.R.T. values (temperature, wear) need administrator rights.",
                         "S.M.A.R.T. 상세 값(온도·마모도)은 관리자 권한이 필요합니다."),
    "svc_filter":   ("Optional only", "권장 항목만"),
    "startup_task_backup": ("Scheduled tasks are exported to XML first, so they can be restored.",
                            "작업 스케줄러 항목은 삭제 전에 XML 로 내보내 두어 되살릴 수 있습니다."),
