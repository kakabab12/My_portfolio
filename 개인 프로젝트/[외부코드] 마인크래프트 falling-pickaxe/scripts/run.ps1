...
try {
    # 어느 폴더에서 실행하든 항상 저장소 루트를 기준으로 동작하도록 고정 (scripts 폴더 안에서 실행해도 안전)
    Set-Location (Split-Path $PSScriptRoot -Parent)

    # Windows often ships a "python" App Execution Alias that just opens the Microsoft Store
    # instead of running anything, even when a real Python is installed. Prefer the "py" launcher,
    # which points at the actual interpreter, and fall back to "python" only if "py" is missing.
