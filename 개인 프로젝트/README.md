...
| 기능 | 내용 |
|---|---|
| **시스템 업데이트** | `winget`·`pip`를 업데이트하고 윈도우 업데이트/MS Store를 자동으로 띄운다. `wmic`으로 **GPU 제조사(NVIDIA/AMD/Intel)를 감지해 해당 드라이버 페이지를 열어준다** |
| **청소 & 부스트** | DNS 캐시 초기화, Temp 삭제, `dism` 이미지 정리, `netsh winsock reset`, 디스크 조각모음 |
| **게임 모드** | 무거운 백그라운드 프로그램을 일괄 종료하고 탐색기를 재시작해 메모리를 회수한다 |
| **핑 테스트** | 8.8.8.8과 1.1.1.1 핑 결과를 **정규식으로 파싱**해 지연시간을 등급으로 평가한다 |
| **PC 정보** | CPU·메인보드·GPU·RAM과 내부/외부 IP를 조회한다 |
...
**직접 만든 것이 아니다.** [vycdev/falling-pickaxe](https://github.com/vycdev/falling-pickaxe)
(GPL-3.0) 프로젝트를 **학습·개조 목적으로** 받아둔 것이다.
Pygame 기반 게임 루프 구조와 YouTube 실시간 연동 방식을 참고하려고 보관했다.

폴더명에 `[외부코드]` 를 붙여 직접 만든 프로젝트와 구분했다.
