...
class MegaTnt(Tnt):
    def __init__(self, space, x, y, texture_atlas, atlas_items, sound_manager, owner_name=None, velocity=0, rotation=0, mass=100, scale_multiplier=2):
    ...
    def explode(self, explosions):
        ...
        # 파티클 개수를 크기에 비례시켜서, 핵(scale=6)처럼 커진 폭발일수록 훨씬 화려해지도록 함
        particle_count = min(20 * self.scale_multiplier, 160)
        self._explode_with_radius(explosions, explosion_radius, self.scale_multiplier, particle_count)
