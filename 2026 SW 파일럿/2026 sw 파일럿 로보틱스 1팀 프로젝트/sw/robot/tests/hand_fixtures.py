...
- 굽힌 손가락 TIP = root-40 (비율 0.8 <= curl_confirm 0.9 + 방향 반전 — 굽힘 확인)
- 카메라를 가리키는 검지(point_camera) = 화면 투영은 뭉치지만 z가 카메라
  쪽으로 길다 — 3D 비율로는 폄
- 짧지만 접힘 증거가 없는 손가락(bunched_flat) = y만 뭉치고 z=0 — 기권
"""
import numpy as np
...
def make_hand_landmarks(shape, root_xy=(500.0, 400.0)):
    ...
    판별 기대값: fist / finger / finger / open / finger / None.
    """
