...
상대 위치 방식으로 조종하는 독립 예제다. 기존 TurtleBot3 제스처 코드에는
의존하지 않는다.

## 동작 대응

기준 자세를 한 번 잡은 뒤, 사람 팔의 변화량을 로봇팔의 현재 안전 자세에 더한다.
...
[`config.json`](config.json)의 해당 `direction`을 `1` ↔ `-1`로 바꾼다.

## 먼저 안전하게 확인

1. 로봇팔을 책상 위의 충돌 없는 **안전 자세**에 놓는다. 손으로 움직일 수 있는
   상태라면 팔을 지지한다.
2. 아래 명령으로 카메라·관절 인식만 확인한다. 이 모드는 USB 모터 포트를 열지
   않으며 화면에 계산된 목표값만 표시한다.
...
3. 카메라 창에서 `C`를 눌러 현재 사람 팔 자세를 기준 자세로 저장한다. 화면의
   `CALIBRATED`를 확인하고 천천히 팔을 움직여 각 목표값이 자연스럽게 변하는지
   본다.

## 실제 로봇팔 연결

현재 USB 연결에서는 TurtleBot3 OpenCR이 `/dev/ttyACM0`으로 확인됐다. 따라서
SO-101 follower controller 후보는 `/dev/ttyACM1`이다. **아래 명령 전에는 케이블을
눈으로 한 번 확인하고, `ttyACM0`을 절대로 SO-101 포트로 쓰지 않는다.**
...
`--enable-arm`을 줘야만 모터 통신 프로세스를 시작한다. 시작할 때 6개 모터의
...
방향을 확인한다. 방향이 반대이면 프로그램을 종료하고 `config.json`의 그 관절
`direction`을 바꾼 뒤 다시 `C`부터 한다.
...
보정값이나 모터 EEPROM을 변경하지 않는다. 파일 이름이 다르면 `--calibration`
옵션으로 지정한다.
