#!/usr/bin/env bash
# 저장 지도 기반 Nav2/AMCL과 A-B-C-D-A 구간별 제어권 전환을 실행한다.
# A->B: AUTO/Nav2, B->C: 제스처, C->D: 조이스틱, D->A: AUTO/Nav2
set -euo pipefail
...
usage() {
    cat <<'EOF'
Usage: bash scripts/run_navigation_gesture_joystick.sh [options]
...
gesture recognition, joystick, Wi-Fi glove, and the velocity mux. P5U is not
...
wait_for_ros_topic_subscriber() {
    # 소음 노드는 OpenCR에 직접 연결하지 않고 /opencr_led_status만 발행한다.
    # 따라서 TurtleBot3 노드의 LED subscriber가 실제 등록됐는지 확인해야
    # 소음 감지만 실행된 채 물리 LED가 반응하지 않는 상황을 막을 수 있다.
    local topic_name="$1"
    local timeout_seconds="$2"
    local deadline=$((SECONDS + timeout_seconds))

    while (( SECONDS < deadline )); do
        if ros2 topic info "$topic_name" 2>/dev/null | awk -F ': *' '
            /Subscription count/ {
                found = 1
                exit ($2 + 0 > 0 ? 0 : 1)
            }
...
cd "$ROOT_DIR"
echo "[1/8] TurtleBot3 bringup + mux 전용 하드웨어 입력 시작 (OpenCR: $USB_PORT)"
...
echo "[3/8] 제스처·조이스틱·Wi-Fi 장갑·Nav2 속도 mux 시작"
/usr/bin/python3 ros2_bridge/cmd_vel_mux.py &
pids+=("$!")

echo "[4/8] Wi-Fi 장갑 입력 시작 (컨트롤러 모드에서만, 앞뒤 반전 적용)"
...
if [[ "$JOYSTICK_AVAILABLE" == true ]]; then
    echo "[5/8] 조이스틱 입력 시작 (/dev/input/js0)"
    ros2 launch "$ROOT_DIR/ros2_bridge/joystick_with_mux.launch.py" &
    pids+=("$!")
else
    echo "[5/8] 조이스틱 입력 생략 (/dev/input/js0 미연결)"
fi

echo "[6/8] 저장 지도 기반 Nav2/AMCL 시작 ($MAP_PATH)"
ros2 launch "$ROOT_DIR/ros2_bridge/navigation_with_mux.launch.py" "map:=${MAP_PATH}" &
pids+=("$!")

echo "Nav2 action 서버 초기 확인 (최대 5초)..."
nav_ready=false
for _ in {1..25}; do
...
if [[ "$nav_ready" != true ]]; then
    # AMCL은 RViz의 2D Pose Estimate(또는 순찰 미션의 A 초기 위치)를 받기
    # 전에는 map->odom TF를 내보내지 않는다. 그 동안 Nav2 lifecycle이
    # navigate_to_pose action을 만들지 않는 것은 정상이며, 여기서 종료하면
    # RViz도 함께 종료되어 초기 위치를 넣을 기회가 사라진다.
    echo "Nav2가 초기 위치를 기다리고 있습니다. 실행은 계속합니다; RViz에서 2D Pose Estimate를 지정하거나 순찰 미션의 A 초기화가 완료될 때까지 기다리세요." >&2
fi

echo "[7/8] 제스처 컨트롤러 시작 (처음 제어권=AUTO/Nav2)"
...
if [[ "$ENABLE_SOUND" == true ]]; then
    echo "OpenCR LED 제어 subscriber 준비 대기..."
    if ! wait_for_ros_topic_subscriber /opencr_led_status 10; then
        echo "TurtleBot3/OpenCR LED 제어 subscriber를 찾지 못했습니다. 물리 LED 없이 소음 감지만 실행하지 않도록 전체 실행을 중단합니다." >&2
        exit 1
    fi

    echo "[8/8] C270 웹캠 마이크 소리 이상감지 + OpenCR LED 시작"
    ...
    # state 메시지는 노드가 정상 동작 중이어도 ROS CLI discovery 시점에 따라
    # 첫 발행을 놓칠 수 있다. 실제로 이 경우 State=NORMAL까지 출력한 노드를
    # "상태 토픽 없음"으로 오판해 전체 실행과 RViz를 종료시켰다. 위의 노드
    # 등록 확인을 통과한 뒤에는 오디오 스트림에 짧은 안정화 시간을 준다.
    echo "C270 오디오 스트림 안정화 대기..."
    sleep 3
    # 오디오 입력을 연 직후의 허브/전원 문제는 LiDAR 스트림이 먼저 끊긴다. 순찰 전에 다시
    # 센서 메시지를 확인한다. 다만 ROS CLI는 부하가 큰 통합 실행에서 이미 흐르는
    # BEST_EFFORT /scan의 discovery를 놓칠 수 있다. 실제 순찰 미션은 아래에서
    # 스캔과 지도의 연속 일치를 다시 검증한 뒤에만 주행하므로, 이 보조 CLI
    # 확인의 오탐으로 전체 실행과 RViz를 종료하지 않는다.
    echo "웹캠 마이크 시작 후 LDS-03 /scan 안정성 재확인..."
    if ! wait_for_ros_topic_message /scan 15 best_effort; then
        echo "경고: ROS CLI가 /scan 메시지를 재확인하지 못했습니다. LiDAR 스트림은 순찰 미션의 지도-스캔 일치 검증에서 다시 확인합니다." >&2
    fi
else
    echo "[8/8] 웹캠 마이크 소음 탐지 생략 (--with-sound으로 활성화)"
fi

if [[ "$START_PATROL" == true ]]; then
    echo "[추가] 구간별 waypoint 미션 시작 (A 초기 위치를 먼저 등록한 뒤 B로 Nav2 주행)"
    /usr/bin/python3 ros2_bridge/waypoint_handoff_mission.py &
    pids+=("$!")
fi

echo
echo "준비 완료: 초기 제어권은 AUTO/Nav2입니다. RViz에서 초기 위치와 2D Goal Pose를 지정하세요."
if [[ "$ENABLE_SOUND" == true ]]; then
...
echo "컨트롤러 ON 중 OK 사인 1.5초=장갑 기울기·중립 여부와 무관하게 수동 제어 OFF 후 AUTO/Nav2 복귀. 종료는 Ctrl+C입니다."
if [[ "$START_PATROL" == true ]]; then
    echo "A->B 도착 완료까지는 항상 AUTO/Nav2로 주행합니다. B 도착 후 짧은 따봉으로 제스처 모드를 켜 C로 이동하세요."
