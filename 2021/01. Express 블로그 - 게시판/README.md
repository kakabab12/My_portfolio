...
Node.js + MongoDB로 만든 **개인 블로그(게시판)** 다. 화면에 "용이의 BLOG"라고
띄운다. 글을 쓰고 읽고, 각 글에 댓글을 다는 기능까지 구현했다.

> **폴더 이름 주의** — 원래 폴더명이 `express-locallibrary-tutorial` 이었다.
> MDN의 Express 튜토리얼(도서관 앱)을 따라 하며 시작했기 때문인데,
> **실제로 만든 것은 도서관이 아니라 블로그/게시판**이다. 튜토리얼로 골격을
>잡은 뒤 내용은 다르게 갔다.
...
즉 **설계(06-09) → 로직(06-11) → 화면(06-15)** 순서로 6일에 걸쳐 만들었다.

---

## 데이터 모델

게시글 안에 댓글을 **내장 문서(embedded document)** 로 넣었다. MongoDB가
관계형 DB와 다른 지점을 처음 체감한 부분이다.
...
여기서는 `$push` 연산으로 게시글 문서 안 배열에 직접 밀어 넣는다.
...
> `복구/`는 원본을 잃어버렸다가 일부만 되살린 흔적이다. 뷰 엔진도 원본(EJS)과
> 달리 pug로 되어 있다. 실제 결과물을 보려면 **`원본/`** 을 봐야 한다.
...
> ⚠ MongoDB가 로컬에 떠 있어야 한다. 접속 주소는 `app.js`에 있다
...
당시 코드를 지금 기준으로 보면 이런 것들이 눈에 띈다. 고치지 않고 **그때
수준을 그대로 남겨** 성장 과정이 보이게 두었다.

- `var` 사용, 콜백 중첩 (지금이라면 `const`/`async-await`)
- 오류 처리가 `console.log` 후 리다이렉트뿐
- `models/comment.js`가 정의만 되고 실제로는 board의 내장 스키마를 쓴다
- 입력값 검증이 없다
