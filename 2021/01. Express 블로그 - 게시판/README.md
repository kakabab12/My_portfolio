...
| 폴더 | 내용 |
|---|---|
| [`원본/`](원본/) | ⭐ **실제로 만든 앱.** 모델·라우터·EJS 화면이 모두 들어 있습니다 |
| [`복구/`](복구/) | Express 제너레이터 골격만 남은 부분 복구본. **모델과 화면이 빠져 있어** 그대로는 안 돌아갑니다 |
...
```bash
cd 원본
npm install          # node_modules는 저장소에 안 올렸습니다
...
지금 기준으로 다시 보면 눈에 걸리는 곳이 있습니다. 고치지 않고 **그때 수준
그대로** 두었습니다. 고쳐 놓으면 뭘 몰랐는지가 안 남습니다.
